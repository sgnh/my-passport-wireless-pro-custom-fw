################################################################################
#
# RTKPACKAGE_RTL8811AU
#
################################################################################

RTKPACKAGE_RTL8811AU_VERSION = 0.0.1
RTKPACKAGE_RTL8811AU_SITE_METHOD = local
RTKPACKAGE_RTL8811AU_DEPENDENCIES = linux
RTKPACKAGE_RTL8811AU_SITE = $(TOPDIR)/project/prjPackages/wifi/rtl8811au/
RTKPACKAGE_RTL8811AU_MAKE_ENV = \
					CROSS="$(TARGET_CROSS)" \
...
define RTKPACKAGE_RTL8811AU_CONFIGURE_CMDS
	sed -i "s/CONFIG_80211W = [y|n]/CONFIG_80211W = y/g" $(@D)/Makefile
	sed -i "s/CONFIG_WIFI_TEST = [y|n]/CONFIG_WIFI_TEST = y/g" $(@D)/Makefile
endef
endif

define RTKPACKAGE_RTL8811AU_BUILD_CMDS
	$(RTKPACKAGE_RTL8811AU_MAKE_ENV) $(MAKE) -C $(@D) all
endef

define RTKPACKAGE_RTL8811AU_INSTALL_TARGET_CMDS
	mkdir -p $(TARGET_DIR)/lib/modules/$(LINUX_VERSION_PROBED)/kernel/drivers/net/wireless/
	$(INSTALL) -p -m 644 $(@D)/8811au.ko $(TARGET_DIR)/lib/modules/$(LINUX_VERSION_PROBED)/kernel/drivers/net/wireless
endef
