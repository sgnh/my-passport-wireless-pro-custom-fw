...
echo $timestamp ": storage_transfer_start_now.sh" $@ >> /tmp/backup.log
...
if [ "$version_api" == "" ]; then
	...
	if [ "$available_chk" == "SD" ]; then
		...
		#	fileNum=`ls -a "${SDmount}" | wc -l`	
		#	if [ "${fileNum}" -eq 2 ]; then
		#		echo "18;0;" > /tmp/MCU_Cmd 
		#		echo "total_size_in_bytes=0" > /tmp/transfer_size_total
		#		echo "transferred_size_in_bytes=0" > /tmp/transfer_size
		#		echo "status=completed" > /tmp/transfer_state
		#		/usr/local/sbin/storage_transfer_status.sh > /dev/null 2>&1 &
		#		/usr/local/sbin/incUpdateCount.pm storage_transfer &
		#		exit 0
		#	fi
		#else
		#	fileNum=`ls -a "${SDmount}" | grep -v "System Volume Information" | wc -l`	
		...
		#	fi
		#else
		#	fileNum=`ls -a "${USBmount}" | grep -v "System Volume Information" | wc -l`	
		#	if [ "${fileNum}" -eq 0 ]; then
		#		echo "18;0;" > /tmp/MCU_Cmd 
		#		echo "total_size_in_bytes=0" > /tmp/transfer_size_total
		#		echo "transferred_size_in_bytes=0" > /tmp/transfer_size
