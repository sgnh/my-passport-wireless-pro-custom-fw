...
plexupdate_progress()
{
     echo "plex_upgrading $1" > /tmp/fw_update_status
     echo "plex_upgrading $1" > /tmp/plexfw_update_status
     if [ $1 -eq 100 ]; then
     	echo "plex_upgraded" > /tmp/fw_update_status
     fi
     sleep 1
     cat /tmp/fw_update_status
...
check_for_install_package()
{
    ...
    if [ ${isInstallPackage} -eq 1 ]; then
        ...
        if [ ${result} != "0" ]; then
            ...
            pre-update_error "plex_failed 200 \"invalid firmware package\""
        fi
...
pkg_upgrade_exec()
{
    ...
    if [ ! -f /tmp/fwupg_images/upgrade.sh ]; then
        update_error "plex_failed 200 \"invalid firmware package\""
    fi
...
pkg_upgrade_verify()
{
    ...
    if [ $? -eq 0 ]; then
        update_error "plex_failed 202 \"upgrade download failure\""
    fi
...
start_update_korra()
{
	...
	tar xvf $1 updatefw -C /tmp/ || pre-update_error "plex_failed 200 \"invalid firmware package\""
	...
	[ $isFAIL -eq 1 ] && pre-update_error "plex_failed 200 \"invalid firmware package\""
...
filename=`/usr/local/sbin/getPackagesFile.sh plexmediaserver`
#check_size=${2:-""}
#test=${3:-""}
# check params
#if [ $# -lt 1 ]; then
#    echo "usage: updatePlexFirmwareFromFile.sh <filename> [check_filesize]"
#    exit 1
#fi
if [ ! -f "${filename}" ]; then
    echo "File not found"
...
[ ! -f /tmp/fw_update_status ] && echo "upgrading" > /tmp/fw_update_status
...
check_for_install_package ${filename}
...
if [ "${check_size}" != "" ]; then
    ...
    if [ "${blocksize_dpkg0}" != "${blocksize_ls}" ] && [ "${blocksize_dpkg1}" != "${blocksize_ls}" ]; then
        error="plex_failed 202 \"failed download file size check\""
        pre-update_error "${error}"
    fi
fi
...
if [ "${master_package_new}" == "wd.container" ] || [ "${master_package_new}" == "wd.group" ] || [ "${master_package_new}" == "wd.supergroup" ]; then
    ...
    if [ "${master_package_name}" != "${master_package_new}" ]; then
            error="plex_failed 200 \"invalid firmware package\""
            pre-update_error $error
    fi
