...
u8 rtw_set_sleep_macid_cmd(_adapter  *padapter, u32 sta_macid, u32 sleep_wakeup)
{
	struct cmd_obj*			ph2c;
	struct setsleepmacid_parm *psta_macid;
	struct cmd_priv 			*pcmdpriv=&padapter->cmdpriv;
	u8	res=_SUCCESS;
...
_func_enter_;	
	...
	psta_macid = (struct setsleepmacid_parm*)rtw_zmalloc(sizeof(struct setsleepmacid_parm)); 
	...
	init_h2fwcmd_w_parm_no_rsp(ph2c, psta_macid, _set_sleep_macid_CMD_);

	psta_macid->sta_macid = sta_macid;
	psta_macid->sleep_wakeup = sleep_wakeup;
	...
	if (!is_primary_adapter(padapter)) {
		if (pmlmepriv->LinkDetectInfo.NumTxOkInPeriod > BusyThresholdLow
			...
			if (is_client_associated_to_ap(padapter)) {
				if (rtw_read32(padapter, REG_EDCA_BK_PARAM) != padapter->backup_acParam_BE) {
				...
				if (rtw_read32(pbuddy_adapter, REG_EDCA_BK_PARAM) != pbuddy_adapter->backup_acParam_BE) {
