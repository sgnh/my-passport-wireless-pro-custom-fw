...
u8 sitesurvey_cmd_hdl(_adapter *padapter, u8 *pbuf)
{
	...
	if ((pmlmeext->sitesurvey_res.state == SCAN_START) || (pmlmeext->sitesurvey_res.state == SCAN_TXNULL))
	{
		...
		if (rtw_odm_adaptivity_needed(padapter) == _TRUE)
			initialgain = 0x28;
...
u8 set_sleep_macid_hdl(_adapter *padapter, u8 *pbuf)
{
	struct setsleepmacid_parm * set_sleep_macid;
	struct mlme_priv		*pmlmepriv = &padapter->mlmepriv;
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	...
	set_sleep_macid = (struct setsleepmacid_parm *)pbuf;
	
	/* use drop to replace sleep */
	rtw_hal_macid_drop(padapter, set_sleep_macid->sta_macid);
