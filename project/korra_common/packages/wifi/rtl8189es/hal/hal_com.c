...
_func_enter_;
	...
	if (enable) {
...
#if 0
		{
			...
			dump_TX_FIFO(padapter, 7, PageSize);
		}
#endif
...
static u8 rtw_hal_set_ap_ps_cmd(_adapter *adapter, u8 enable)
{
	...
	ret = pHalFunc->fill_h2c_cmd(adapter, H2C_SAP_PS_,
		...
		PRSVDPAGE_LOC rsvdpageloc)
{
	...
	ret = pHalFunc->fill_h2c_cmd(padapter, H2C_BCN_RSVDPAGE,
	...
	ret = pHalFunc->fill_h2c_cmd(padapter, H2C_PROBERSP_RSVDPAGE,
...
void rtw_hal_set_fw_ap_wow_related_cmd(_adapter* padapter, u8 enable)
{
...
#ifdef CONFIG_P2P_WOWLAN
	if(pwrctl->wowlan_p2p_mode == _TRUE) {
	...
	case HW_VAR_WOWLAN:
	{
		...
		case HW_VAR_AP_WOWLAN:
		{
			...
			case WOWLAN_AP_ENABLE:
				...
				rtw_hal_set_fw_ap_wow_related_cmd(adapter, 1);
			...
			case WOWLAN_AP_DISABLE:
				...
				rtw_hal_set_fw_ap_wow_related_cmd(adapter, 0);
...
static void rtw_set_agg_by_mode(_adapter *padapter, u8 cur_wireless_mode)
{
	u8 valueDMATimeout;
...
void dm_DynamicSDIOTxAgg(_adapter *padapter, u8 from_timer)
{
	struct dvobj_priv	*pdvobjpriv = adapter_to_dvobj(padapter);
	...
	if (is_client_associated_to_ap(padapter)) {
		...
		rtw_set_agg_by_mode(padapter, cur_wireless_mode);
	}
}
