	...
	if(Adapter->iface_type == IFACE_PORT1)
	{
		...
		else if(mode == _HW_STATE_AP_)
		{
			...
			if (Adapter->registrypriv.wifi_spec)
				rtw_write32(Adapter, REG_RCR, (0x7000208e& ~(RCR_CBSSID_BCN)));//CBSSID_DATA must set to 0,Reject ICV_ERROR packets
			else
...
#endif //CONFIG_CONCURRENT_MODE
	{
		...
		if((mode == _HW_STATE_STATION_) || (mode == _HW_STATE_NOLINK_))
		{
...
#endif //CONFIG_CONCURRENT_MODE
			{
				...
				#endif /*!defined(CONFIG_CONCURRENT_MODE) && defined(CONFIG_INTERRUPT_BASED_TXBCN_EARLY_INT)*/
...
_func_enter_;
	...
	switch (variable) {
		...
		case HW_VAR_AC_PARAM_VO:
			adapter->backup_acParam_VO = ((u32 *)(val))[0];
			rtw_write32(adapter, REG_EDCA_VO_PARAM, ((u32 *)(val))[0]);
			break;
		case HW_VAR_AC_PARAM_VI:
			adapter->backup_acParam_VI = ((u32 *)(val))[0];
			rtw_write32(adapter, REG_EDCA_VI_PARAM, ((u32 *)(val))[0]);
			break;
		case HW_VAR_AC_PARAM_BE:
			pHalData->AcParam_BE = ((u32 *)(val))[0];
			adapter->backup_acParam_BE = ((u32 *)(val))[0];
			rtw_write32(adapter, REG_EDCA_BE_PARAM, ((u32 *)(val))[0]);
			break;
		case HW_VAR_AC_PARAM_BK:
			adapter->backup_acParam_BK = ((u32 *)(val))[0];
		...
		case HW_VAR_MACID_DROP:
		{
			...
			if (id < 32){
				reg_macid_sleep = REG_MACID_NO_LINK_0;
				bit_shift = id;
			} else if (id < 64) {
				reg_macid_sleep = REG_MACID_NO_LINK_1;
				bit_shift = id-32;
			} else {
				rtw_warn_on(1);
				break;
