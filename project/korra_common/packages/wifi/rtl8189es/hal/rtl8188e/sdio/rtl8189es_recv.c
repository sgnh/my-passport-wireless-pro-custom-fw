...
s32 rtl8188es_init_recv_priv(PADAPTER padapter)
{
	...
	netif_napi_add(padapter->pnetdev, &padapter->napi, recv_napi_poll, RTL_NAPI_WEIGHT);
#else
...
#ifdef CONFIG_SDIO_RX_COPY
#ifdef CONFIG_NAPI
int sdio_napi_recv(_adapter *padapter, int budget)
{
	PHAL_DATA_TYPE		pHalData;
	struct recv_priv		*precvpriv;
	struct recv_buf		*precvbuf;
	...
	while ( (work_done < budget) &&
		...
		if ((padapter->bDriverStopped == _TRUE)||(padapter->bSurpriseRemoved== _TRUE)) {
			DBG_8192C("sdio_napi_recv => bDriverStopped or bSurpriseRemoved \n");
			rtw_skb_free(pskb);
			goto exit;
			break;
		}
	...
	do {
		...
		do {
			...
			if (precvframe == NULL) {
				...
				// The case of can't allocte recvframe should be temporary,
				// schedule again and hope recvframe is available next time.
				bschedule_napi_poll = _TRUE;
				goto check_napi_skb_queue;
				break;
			}
...
int recv_napi_poll(struct napi_struct *napi, int budget)
{
	...
	work_done = sdio_napi_recv(padapter, budget);
