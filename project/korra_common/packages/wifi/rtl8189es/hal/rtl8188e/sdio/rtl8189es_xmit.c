...
//			_enter_critical(&pframe_queue->lock, &irqL1);
			...
			while (rtw_is_list_empty(frame_phead) == _FALSE)
			{
				...
				if ((pxmitframe->attrib.triggered_by_pspoll == 1) && (IS_MCAST(pxmitframe->attrib.ra) == _FALSE)) {

					psta = pxmitframe->attrib.psta;
					if (psta) {
						/*ATOMIC_DEC(&psta->pspoll_op);*/
						wait_for_pspoll = _TRUE;
						pxmitframe->ack_report = 1;				
					}

					pxmitframe->attrib.triggered_by_pspoll = 0;
				}
#endif				
		...
		if (pxmitbuf)
		{
			...
			if (pxmitbuf->len > 0 && pxmitbuf->priv_data) {
				...
				if (wait_for_pspoll == _FALSE) {
					...
					enqueue_pending_xmitbuf(pxmitpriv, pxmitbuf);
					if (padapter->registrypriv.wifi_spec == 1)
						rtw_ack_tx_wait(pxmitpriv, 2000); /*wait 40 ms*/
					else
						rtw_ack_tx_wait(pxmitpriv, 40); /*wait 40 ms*/
