...
s32 rtw_hal_macid_drop(PADAPTER padapter, u8 macid)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	...
	rtw_hal_set_hwreg(padapter, HW_VAR_MACID_DROP, &macid);
	...
	//polling
	do {
		value = rtw_read8(padapter, REG_TXPKT_EMPTY);
		if (value == 0xff)
			break;
	...
	if(!count)
		printk("count=%d , value=%02x, check TX empty fail\n", count, value);
		
	rtw_hal_set_hwreg(padapter, HW_VAR_MACID_DROP_CANCEL, &macid);
