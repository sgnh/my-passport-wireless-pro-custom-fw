...
#ifdef CONFIG_MCC_MODE
/* common mcc data for all adpater */
struct mcc_obj_priv {
	//u8 do_mcc; /* check if driver is doing MCC, set false under sitesurvey, set true not under sitesurvey */
	u8 duration; /* channel stay period, UNIT:1TU */
	u8 mcc_c2h_status;
	u8 mcc_locCTS;
	u8 mcc_locNulldata;
	//u8 need_mcc; /* clear only when sta disconnects from AP or SoftAP/GO stops */
	u8 mcc_status; 
	u32 mcc_launch_time;
	_mutex mcc_mutex;
	_lock mcc_lock;
	struct submit_ctx mcc_sctx;
	u8 cur_mcc_success_cnt; /* used for check mcc switch channel sucess */
	u8 prev_mcc_success_cnt; /* used for check mcc switch channel sucess */
	u8 mcc_tolerance_time; /* used for detect mcc switch channel sucess */
	PADAPTER iface[2]; /* sort by order, 2 for current iface number */
};

/*  mcc data for adpater */
struct mcc_adapater_priv {
 	u8 order; 
	u8 role;
...
struct _ADAPTER{
	...
	// Definition by Arvin 2015/05/29
	// This parameter represents the state of net_device currently.	
	u8 netif_up;
...
#ifdef CONFIG_MCC_MODE
	struct mcc_adapater_priv mcc_adapterpriv;
 #endif /* CONFIG_MCC_MODE */
...
#endif
	u32 backup_acParam_BK;
	u32 backup_acParam_BE;
	u32 backup_acParam_VI;
	u32 backup_acParam_VO;
