...
#define MCC_STATUS_START_PROCESS_MCC_CMD	BIT0
#define MCC_STATUS_STOP_PROCESS_MCC_CMD BIT1
#define MCC_STATUS_NEED_MCC	BIT2
#define MCC_STATUS_DOING_MCC	BIT3


#define MCC_DURATION 35
#define MCC_NOCLIENT_DURATION 70
#define MCC_SWCH_FW_NOTIFY_TIME 10
...
enum mcc_status_rpt{
	...
	MCC_RPT_STOPMCC			= 2,
	MCC_RPT_READY				= 3,
	MCC_RPT_SWICH_NOTIFY	= 7,
	MCC_RPT_MAX				,
};

/* H2C command index = 0x15 */
struct mcc_update_info_param {  
	    u8 order:4;
	    u8 rsvd:4;
	    ...
	    u8 distxnull:1;
	    u8 c2hrpt:2; 			/* 0: disable, 1: report when Fail, 2: report all */
	    u8 rsvd2:4;
} ;
...
/* H2C command index = 0x18 */
struct mcc_info_param{
	u8 order:4; 		/* user index, (0: default ) */
	u8 totalnum:4; 	/*max user number (ch number). If totalnum=0xf, fw switch channel to default setting(stop MCC mode).*/
	/*
		if channelscan=0-> channel index, 
		channelscan=1->Scan channel rsvdpage location, channel combination in the rsvdpage.
	*/
	u8 chidx;
	u8 bw:2; 		/* Data2[1:0]: 20MHz, 1: 40MHz, 2: 80MHz */
	u8 bw40sc:3;
	u8 bw80sc:3;
	u8 duration; 		/* channel stay period, UNIT:1TU  */
	u8 role:3; 		/* 0: STA, 1: AP, 2: GC, 3: GO */
	/* 1: if null pkt retry fail, keep in current channel,  0: if null pkt retry fail, switch to next channel */
	u8 InCurCh:1;	/* 1: if null pkt retry fail, keep in current channel,  0: if null pkt retry fail, switch to next channel */
	u8 rsvd0:4;
	u8 rsvd1 ;
	u8 rfetype:4; 	/* 8812A only */
	u8 distxnull:1; 	/* No tx null pkt before switch channel */
	u8 c2hrpt:2; 		/* c2h report type, 0: disable, 1: report when Fail, 2: report all */
	u8 channelscan:1;	/* it means that this order is channel scan */
	
};

/* H2C command index = 0x16 */
/* bitmap0 for AP/GO, bitmap1 for STA/GC  */
struct mcc_macid_bitmap{  
...
/* H2C command index = 0x19 */
struct mcc_noa_param{
	u8 mcc_noa_fw_enabe:1;
	u8 mcc_noa_tsf_offset:7;
	u8 mcc_noa_start_time;
	u8 mcc_noa_duration;
	u8 mcc_noa_count;
	u8 swchannel_notify_time;
};

struct mcc_c2h_notify{
	enum mcc_status_rpt		status;
	u8		channel;
};
...
void rtw_hal_mcc_stop_TX_handler(void *FunctionContext);
