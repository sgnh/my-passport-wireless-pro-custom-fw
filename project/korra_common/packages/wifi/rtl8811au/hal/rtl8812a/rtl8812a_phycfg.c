...
)
{
		...
		) 
	{
		...
		if (rtw_hal_check_mcc_status(Adapter, MCC_STATUS_NEED_MCC))  {
			DBG_871X("MCC, don't IQK\n");
			...
			if (_TRUE == Adapter->registrypriv.en_mcc) {
				struct mcc_adapater_priv *pmccadapriv = &Adapter->mcc_adapterpriv;
				u32 regc10 = 0;
				
				PHY_SetBBReg(Adapter, 0x82c, BIT(31), 0x1);
				pmccadapriv->TX_X = (u16)(rtw_read32(Adapter, 0xcd4) & 0x000007ff);
				pmccadapriv->TX_Y = (u16)(rtw_read32(Adapter, 0xccc) & 0x000007ff);
				if (0)
					DBG_871X("TX_X = 0x%02x, TX_Y = 0x%02x\n", pmccadapriv->TX_X, pmccadapriv->TX_Y);
				
				PHY_SetBBReg(Adapter, 0x82c, BIT(31), 0x0);
				regc10 = rtw_read32(Adapter, 0xc10);
				pmccadapriv->RX_X =  (u16) (regc10 & 0x000003ff);
				pmccadapriv->RX_Y =  (u16) ((regc10 & 0x03ff0000)>>16);
				if (0)
					DBG_871X("RX_X = 0x%02x, RX_Y = 0x%02x\n", pmccadapriv->RX_X, pmccadapriv->RX_Y);
