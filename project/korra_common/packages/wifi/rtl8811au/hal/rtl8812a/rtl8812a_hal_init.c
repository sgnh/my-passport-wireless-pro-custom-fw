	...
	)
{
	...
	DBG_871X("%s: BTCoexist=%s, AntNum=%d\n, ", __FUNCTION__,pHalData->EEPROMBluetoothCoexist==_TRUE?"Enable":"Disable" 
	...
	)
{
	...
	if (padapter->registrypriv.wifi_spec == 1) {
		padapter->mlmepriv.ChannelPlan = RT_CHANNEL_DOMAIN_REALTEK_DEFINE;
		DBG_871X("WIFI_SPEC:force channel plan:0x%02x\n", padapter->mlmepriv.ChannelPlan);
	}
...
extern void UpdateInterruptMask8812AU(PADAPTER padapter,u8 bHIMR0 ,u32 AddMSR, u32 RemoveMSR);
...
#endif // CONFIG_CONCURRENT_MODE
	{
		...
		if((mode == _HW_STATE_STATION_) || (mode == _HW_STATE_NOLINK_))
		{
...
#endif // CONFIG_CONCURRENT_MODE
			{
				...
				#if defined(CONFIG_CONCURRENT_MODE) && defined(CONFIG_INTERRUPT_BASED_TXBCN_EARLY_INT)
					if (Adapter->registrypriv.wifi_spec == 1) {
						rtw_write8(Adapter, REG_DRVERLYINT, 0x05);/*restore early int time to 5ms*/
						UpdateInterruptMask8812AU(Adapter,_TRUE, 0, IMR_BCNDMAINT0_8812);
					}
		...
		else if(mode == _HW_STATE_AP_)
		{
			...
			//rtw_write8(Adapter, REG_BCN_MAX_ERR, 0xFF);
			if (Adapter->registrypriv.wifi_spec == 1)
				rtw_write8(Adapter, REG_ATIMWND, 0x10); // 16ms
			else
...
static void hw_var_set_mlme_sitesurvey(PADAPTER Adapter, u8 variable, u8* val)
{
	...
	if(*((u8 *)val))//under sitesurvey
	{
		...
		if (rtw_linked_check(Adapter->pbuddy_adapter) && 
			...
			rtw_write8(Adapter, buddy_reg_bcn_ctl, rtw_read8(Adapter->pbuddy_adapter, buddy_reg_bcn_ctl)|DIS_TSF_UDT);
	...
	{
		/*
               * 1. enable to rx data frame
               * 2. config RCR to not receive different BSSID
		  * 3. doesn't enable TSF update &  buddy TSF right now to avoid HW conflic
		  */
		...
		if (Adapter->registrypriv.wifi_spec && check_fwstate(pmlmepriv, WIFI_AP_STATE))
			value_rcr &= ~(RCR_CBSSID_BCN);
