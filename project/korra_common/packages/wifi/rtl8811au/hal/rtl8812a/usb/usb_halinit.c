...
HAL_INIT_PROFILE_TAG(HAL_INIT_STAGES_MISC02);
	...
	rtw_write16(Adapter, REG_PKT_VO_VI_LIFE_TIME, 0x3000);	// unit: 256us. 256ms
	rtw_write16(Adapter, REG_PKT_BE_BK_LIFE_TIME, 0x3000);	// unit: 256us. 256ms
...
{
	EEPROM_EFUSE_PRIV *pEEPROM = GET_EEPROM_EFUSE_PRIV(Adapter);
	u8 bc_addr[ETH_ALEN] = {0xff,0xff,0xff,0xff,0xff,0xff}; /* no pg efuse mac addr */ 

	if(_FALSE == AutoloadFail && _FALSE == _rtw_memcmp(bc_addr, &PROMContent[EEPROM_MAC_ADDR_8821AU], ETH_ALEN))  
	{
		if(IS_HARDWARE_TYPE_8812AU(Adapter))
		{
