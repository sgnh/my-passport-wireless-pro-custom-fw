...
{	
	...
	// more data bit
	if (pxmitframe->attrib.mdata == 1)
		SET_TX_DESC_MORE_DATA_8812(ptxdesc, 1);
	...
	else if((pxmitframe->frame_tag&0x0f)== MGNT_FRAMETAG)
	{
		...
		if (pxmitframe->ack_report) {
                ...
                _adapter *primay_adapter = GET_PRIMARY_ADAPTER(padapter);
            
                //DBG_871X("SW define value:0x%02x\n", primay_adapter->sw_define_val);
            SET_TX_DESC_SW_DEFINE_8812(ptxdesc, primay_adapter->sw_define_val);
...
//			rtw_free_xmitframe(pxmitpriv, pxmitframe);
	...
	pxmitbuf->triggered_by_pspoll = 0;
	pxmitbuf->psta = NULL;
	if (pxmitframe->attrib.triggered_by_pspoll == 1) {
		
		pxmitbuf->triggered_by_pspoll = 1;
		pxmitbuf->psta = pxmitframe->attrib.psta;
	}
#endif	
...
#ifdef CONFIG_AP_MODE
		if (pxmitbuf->triggered_by_pspoll == 1)
			break;
#endif		
...
//DBG_8192C("==> %s \n",__FUNCTION__);
	...
	if (check_buddy_fwstate(padapter, _FW_UNDER_SURVEY|_FW_UNDER_LINKING) == _TRUE) {
		if ((rtw_p2p_chk_role(&padapter->wdinfo, P2P_ROLE_GO) == _TRUE)
				|| check_fwstate(&padapter->mlmepriv, WIFI_AP_STATE) == _TRUE) {
#ifdef CONFIG_AP_TX_UNDER_SCAN
			if (!(rtw_get_oper_ch(padapter) == padapter->pbuddy_adapter->mlmeextpriv.cur_channel
				&& rtw_get_oper_bw(padapter) == padapter->pbuddy_adapter->mlmeextpriv.cur_bwmode
					&& rtw_get_oper_choffset(padapter) == padapter->pbuddy_adapter->mlmeextpriv.cur_ch_offset))
					goto enqueue;
#else /* !CONFIG_AP_TX_UNDER_SCAN */
					goto enqueue;
#endif /* CONFIG_AP_TX_UNDER_SCAN */
		} else	
			goto enqueue;
	}
	...
	if(padapter->registrypriv.en_mcc) {
		if (rtw_hal_check_mcc_status(padapter, MCC_STATUS_DOING_MCC)) {
			if (padapter->mcc_adapterpriv.mcc_tx_stop) 
				goto enqueue;
		}
