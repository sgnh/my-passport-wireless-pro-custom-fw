...
)
{
    ...
    _adapter *primay_adapter = GET_PRIMARY_ADAPTER(Adapter);

        if (primay_adapter->sw_define_val != GET_8812_C2H_TX_RPT_SW_DEFINE(CmdBuf)) {
            //DBG_871X("primay_adapter->sw_define_val(0x%02x) != C2H SW_DEFINE(0x%02x)\n",
                //primay_adapter->sw_define_val, GET_8812_C2H_TX_RPT_SW_DEFINE(CmdBuf));
                return;
            } else
...
//			MPTBT_FwC2hBtMpCtrl(Adapter, tmpBuf, c2hCmdLen);
		...
		case C2H_8812_MCC:
			...
			rtw_hal_c2h_mcc_handler(Adapter, c2hCmdLen, tmpBuf);
			break;
