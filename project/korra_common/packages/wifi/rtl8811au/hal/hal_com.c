	...
	u32		*pLength)
{
	...
	*(pframe + 2) = 0x00;//((padapter->pbuddy_adapter->mcc_duration-15) * 1024) & 0x00ff;//0x00
	*(pframe + 3) = 0x78;//(((padapter->pbuddy_adapter->mcc_duration-15) * 1024)>>8) & 0x00ff;//0x78;
	/* frame recvaddr, length = 6 */
	_rtw_memcpy((pframe + 4), broadcast_addr, ETH_ALEN);
	_rtw_memcpy((pframe + 4 + ETH_ALEN), myid(&(padapter->eeprompriv)), ETH_ALEN);
	_rtw_memcpy((pframe + 4 + ETH_ALEN*2), myid(&(padapter->eeprompriv)), ETH_ALEN);
	*pLength = 22;

}
void rtw_hal_set_mcc_fw_rsvd_page(_adapter* adapter, u8 *pframe, u16 *index,
		u8 tx_desc, u32 page_size, u8 *page_num, u32 *total_pkt_len,
		RSVDPAGE_LOC* rsvd_page_loc)
{
		u32 ctslen = 0; 
		u8 CurtPktPageNum = 0;
		struct mcc_obj_priv *pmccobjpriv = &(adapter_to_dvobj(adapter)->mcc_objpriv);
		
		rsvd_page_loc->LocCTS= *page_num;
		
		DBG_871X("LocCTS: %d\n", rsvd_page_loc->LocCTS);
		rtw_hal_construct_CTS(adapter, &pframe[*index], &ctslen);

		rtw_hal_fill_fake_txdesc(adapter, &pframe[*index-tx_desc],
				ctslen, _FALSE, _FALSE, _TRUE);

		CurtPktPageNum = (u8)PageNum(tx_desc + ctslen, page_size);

		*page_num += CurtPktPageNum;

		*index += (CurtPktPageNum * page_size);

		*total_pkt_len = *index + ctslen;
		pmccobjpriv->mcc_locCTS = rsvd_page_loc->LocCTS;
		pmccobjpriv->mcc_locNulldata= rsvd_page_loc->LocNullData;
}
...
#ifdef CONFIG_MCC_MODE
	struct mlme_priv *pbuddymlmepriv = &adapter->pbuddy_adapter->mlmepriv;
	struct mlme_ext_priv	*pbuddymlmeext = &adapter->pbuddy_adapter->mlmeextpriv;
	struct mlme_ext_info	*pbuddymlmeinfo = &pbuddymlmeext->mlmext_info;
#endif
...
#ifdef CONFIG_MCC_MODE
	if (_TRUE == adapter->registrypriv.en_mcc) {
		if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)){
			...
			get_my_bssid(&pmlmeinfo->network),
			_FALSE, 0, 0, _FALSE);
		} else if  ( check_fwstate(pbuddymlmepriv, WIFI_STATION_STATE)){
			rtw_hal_construct_NullFunctionData(
			adapter->pbuddy_adapter,
			&ReservedPagePacket[BufIndex],
			&NullDataLength,
			get_my_bssid(&pbuddymlmeinfo->network),
			_FALSE, 0, 0, _FALSE);
		}
...
#ifdef CONFIG_MCC_MODE
	if (_TRUE == adapter->registrypriv.en_mcc) {
		rtw_hal_set_mcc_fw_rsvd_page(adapter, ReservedPagePacket,
...
void rtw_hal_dump_mcc_info(struct mcc_info_param *pmcc_info)
{
	DBG_871X("order: %d\n", pmcc_info->order);
	DBG_871X("totalnum: %d\n", pmcc_info->totalnum);
	DBG_871X("chidx: %d\n", pmcc_info->chidx);
	...
	DBG_871X("InCurCh: %d\n", pmcc_info->InCurCh);
	DBG_871X("rfetype: %d\n", pmcc_info->rfetype);
	DBG_871X("distxnull: %d\n", pmcc_info->distxnull);
	DBG_871X("c2hrpt: %d\n", pmcc_info->c2hrpt);
	DBG_871X("channelscan: %d\n", pmcc_info->channelscan);
}

void rtw_hal_dump_macid_bmp(struct mcc_macid_bitmap *pmcc_macid_bmp)
{
	DBG_871X("macid_bitmap0_low: 0x%2x\n", pmcc_macid_bmp->macid_bitmap0_low);
	DBG_871X("macid_bitmap0_high: 0x%2x\n", pmcc_macid_bmp->macid_bitmap0_high);
	DBG_871X("macid_bitmap1_low: 0x%2x\n", pmcc_macid_bmp->macid_bitmap1_low);
	DBG_871X("macid_bitmap1_high: 0x%2x\n", pmcc_macid_bmp->macid_bitmap1_high);
	DBG_871X("macid_bitmap2_low: 0x%2x\n", pmcc_macid_bmp->macid_bitmap2_low);
	DBG_871X("macid_bitmap2_high: 0x%2x\n", pmcc_macid_bmp->macid_bitmap2_high);
}

void rtw_hal_dump_update_mcc_info(struct mcc_update_info_param *pupdate_info)
{
	DBG_871X("order: %d\n", pupdate_info->order);
	if (_TRUE == pupdate_info->mask_duration)
		DBG_871X("duration: %d\n", pupdate_info->duration);
	
	if (_TRUE == pupdate_info->mask_InCurCh)
		DBG_871X("InCurCh: %d\n", pupdate_info->InCurCh);

	if (_TRUE == pupdate_info->mask_distxnull)
		DBG_871X("distxnull: %d\n", pupdate_info->distxnull);

	if (_TRUE == pupdate_info->mask_c2hrpt)
		DBG_871X("c2hrpt: %d\n", pupdate_info->c2hrpt);
}

void rtw_hal_dump_mcc_noa_param(struct mcc_noa_param *pnoa_param)
{
	DBG_871X("mcc_noa_tsf_offset: %d\n", pnoa_param->mcc_noa_tsf_offset);
	DBG_871X("mcc_noa_start_time: %d\n", pnoa_param->mcc_noa_start_time);
	DBG_871X("mcc_noa_duration: %d\n", pnoa_param->mcc_noa_duration);
	DBG_871X("mcc_noa_count: %d\n", pnoa_param->mcc_noa_count);
	DBG_871X("swchannel_notify_time: %d\n", pnoa_param->swchannel_notify_time);
}
...
void rtw_hal_mcc_sw_ch_fw_notify_hdl(PADAPTER padapter)
{
	struct dvobj_priv *pdvobjpriv = adapter_to_dvobj(padapter);
	struct mcc_obj_priv *pmccobjpriv = &(adapter_to_dvobj(padapter)->mcc_objpriv);
	struct mcc_adapater_priv *cur_mccadapriv= NULL, *next_mccadapriv = NULL;
	...
	u8 i = 0, iface_num = pdvobjpriv->iface_nums;
	static u8 cnt = 1;
	u32 single_tx_cri = 5;
	...
	for (i = 0; i < iface_num; i++) {
		...
		if (cur_op_ch == iface->mlmeextpriv.cur_channel) {
			...
			next_iface = iface->pbuddy_adapter;
...
void rtw_hal_c2h_mcc_handler(PADAPTER padapter, u8 buflen, u8 *tmpBuf)
{
	struct dvobj_priv	*pdvobjpriv = adapter_to_dvobj(padapter);
	struct mcc_obj_priv *pmccobjpriv = &(adapter_to_dvobj(padapter)->mcc_objpriv);
	struct mcc_adapater_priv *pmccadapriv = &padapter->mcc_adapterpriv;
	PADAPTER pbuddyadapter = padapter->pbuddy_adapter;
	struct mcc_adapater_priv *pbuddymccadapriv = &pbuddyadapter->mcc_adapterpriv;
	u8 currnet_order = 0;
	static u8 mcc_swch_fw_notify_cnt = 0;
	...
	switch(pmccobjpriv->mcc_c2h_status) {
		...
		_enter_critical_bh(&pmccobjpriv->mcc_lock, NULL);
		pmccobjpriv->cur_mcc_success_cnt++;
		_exit_critical_bh(&pmccobjpriv->mcc_lock, NULL);
			break;
		...
		rtw_sctx_done(&mcc_sctx);
			break;
		case MCC_RPT_SWICH_NOTIFY:
		pdvobjpriv->oper_channel = tmpBuf[1];
		mcc_swch_fw_notify_cnt ++;
		/* DBG_871X("[MCC] SWICH NOTIFY\n"); */

		rtw_hal_mcc_sw_ch_fw_notify_hdl((void *)padapter);
			break;
		default:
		DBG_871X("[MCC] Error MCC status\n");
			break;
	}

}

void rtw_hal_mcc_IQK_offload(PADAPTER padapter)
{
	struct mcc_adapater_priv *pmccadapriv = &padapter->mcc_adapterpriv;
       u8      u1H2CIQKParm[H2C_MCC_IQK_PARAM_LEN]={0}, bready = 0, i=0;
       u16 TX_X = pmccadapriv->TX_X;/* [10:0]  */
       u16 TX_Y = pmccadapriv->TX_Y;/* [10:0]  */
       u16 RX_X = pmccadapriv->RX_X;/* [10:1]  */
      	u16 RX_Y = pmccadapriv->RX_Y;/* [10:1]  */
	
 
       DBG_871X(FUNC_ADPT_FMT"\n", FUNC_ADPT_ARG(padapter));
       
       if (1 == padapter->mcc_adapterpriv.order)
               bready = 1;
 
	u1H2CIQKParm[0] = bready | ((padapter->mcc_adapterpriv.order << 1) & 0x06);
       
      	u1H2CIQKParm[1] = (u8)(RX_X & 0xff);
      	u1H2CIQKParm[2] = ((u8)((RX_X>>8) & 0x03)) |  (u8)((RX_Y<<2) & 0xfc);
       u1H2CIQKParm[3] = (u8)((RX_Y>>6) & 0x0f);
 
      	u1H2CIQKParm[4] = (u8)(TX_X & 0xff);
       u1H2CIQKParm[5] = ((u8)((TX_X>>8) & 0x07)) | (u8)((TX_Y<<3) & 0xf8);
       u1H2CIQKParm[6] = (u8)((TX_Y>>5) & 0x3f);

 #ifdef CONFIG_MCC_MODE_DEBUG
       DBG_871X("u1H2CIQKParm: ");
       for (i=0; i<H2C_MCC_IQK_PARAM_LEN; i++)
               printk("0x%x ", u1H2CIQKParm[i]);
       printk("\n");
 ...
 #endif /* CONFIG_MCC_MODE_DEBUG */
       rtw_hal_fill_h2c_cmd(padapter, H2C_MCC_IQK_PARAM, H2C_MCC_IQK_PARAM_LEN, u1H2CIQKParm);

}

void rtw_hal_set_mcc_macid(PADAPTER padapter, u8 bstop)
{
	struct dvobj_priv	*pdvobjpriv = adapter_to_dvobj(padapter);
	struct mcc_obj_priv *pmccobjpriv = &(pdvobjpriv->mcc_objpriv);
	struct mcc_adapater_priv *pmccadapriv = &padapter->mcc_adapterpriv;

	if (pmccadapriv->role == MCC_ROLE_AP || pmccadapriv->role == MCC_ROLE_GO) {
		u8 macid = 0;
		struct macid_ctl_t *macid_ctl = dvobj_to_macidctl(pdvobjpriv);
		u16 media_status_rpt = 0;
		
		if (_FALSE == bstop) {
			/* 
			  *	Due to Auth/Assoc rsp in VO queue under MCC mode, 
			  *	so we need use this H2C command to let HW TX data in data queue for these macid
			  */
			for (macid=8; macid<=15; macid++) {
				media_status_rpt = (u16)((macid<<8)|0x01); /*  MACID|OPMODE:1 connect */			
				rtw_hal_set_hwreg(padapter,HW_VAR_H2C_MEDIA_STATUS_RPT,(u8 *)&media_status_rpt);	

			}
		} else {
			/* restore MEDIA_STATUS_RPT for unused macid */
			
			for (macid = 8; macid<=15; macid++) {
				if (!rtw_macid_is_used(macid_ctl, macid)) {
					media_status_rpt = (u16)((macid<<8)|0x00); /* MACID|OPMODE:0 means disconnect */		
					rtw_hal_set_hwreg(padapter,HW_VAR_H2C_MEDIA_STATUS_RPT,(u8 *)&media_status_rpt);	
				}
					
			}
		}
	} else if (pmccadapriv->role == MCC_ROLE_STA || pmccadapriv->role == MCC_ROLE_GC) {
		
	} else
		DBG_871X("Error MCC role\n");
		
}

u8 rtw_hal_isneed_mcc(PADAPTER padapter)
{

	struct dvobj_priv	*pdvobjpriv = adapter_to_dvobj(padapter);
	struct mcc_obj_priv *pmccobjpriv = &(adapter_to_dvobj(padapter)->mcc_objpriv);
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_priv	*pbuddymlmeext = &padapter->pbuddy_adapter->mlmeextpriv;
	u8 primary_ch = pmlmeext->cur_channel;
	u8 buddy_primary_ch = pbuddymlmeext->cur_channel;

	/* Must start MCC for different primary channel */
	if (primary_ch != buddy_primary_ch) 
		//pmccobjpriv->need_mcc = _TRUE;
		rtw_hal_set_mcc_status(padapter, MCC_STATUS_NEED_MCC);

	/* check offset & bandwidth for the same primary channel */
	else {
		/* 5G HT40 & VHT80 channel set is fixed, so we don't start MCC for the same primary channel */
		if (primary_ch > 14) {
			//pmccobjpriv->need_mcc = _FALSE;
			rtw_hal_clear_mcc_status(padapter, MCC_STATUS_NEED_MCC);
		}
		/* 2.4G HT40  channel set is not fixed, so we do check channel offset */
		else {
			if (CHANNEL_WIDTH_40 == pmlmeext->cur_bwmode && 
				CHANNEL_WIDTH_40 == pbuddymlmeext->cur_bwmode) {
				/* start MCC for channel offset is not the same */
				if (pmlmeext->cur_ch_offset != pbuddymlmeext->cur_ch_offset)
					//pmccobjpriv->need_mcc = _TRUE; 
					rtw_hal_set_mcc_status(padapter, MCC_STATUS_NEED_MCC);
				else
					//pmccobjpriv->need_mcc = _FALSE;
			...
			/* 
			  for (HT40,HT20) & (HT20,HT20),  primary channel is the same, so HT40 channel must contain HT20 channel
			  the same reason for (HT20,HT20).
			  We don't start MCC for these two setting
			  */
			else {
				//pmccobjpriv->need_mcc = _FALSE; 
				rtw_hal_clear_mcc_status(padapter, MCC_STATUS_NEED_MCC);
...
exit:
	return rtw_hal_check_mcc_status(padapter, MCC_STATUS_NEED_MCC);
}
...
static void rtw_hal_config_mcc_role_setting(PADAPTER padapter)
{
	...
	case MCC_ROLE_GC:
		...
		if (psta) {
			...
			DBG_871X(FUNC_ADPT_FMT":AP/GO staion info is NULL\n", FUNC_ADPT_ARG(padapter));
			rtw_warn_on(1);
		}
		break;
	case MCC_ROLE_AP:
...
u8 rtw_hal_decide_mcc_role(PADAPTER padapter)
{
	struct dvobj_priv	*pdvobjpriv = adapter_to_dvobj(padapter);
	struct mcc_obj_priv *pmccobjpriv = &(pdvobjpriv->mcc_objpriv);
	struct mcc_adapater_priv *pmccadapriv = &padapter->mcc_adapterpriv;
	...
	else if (is_client_associated_to_ap(padapter)) /* after on assoc rsp, WIFI_FW_ASSOC_SUCCESS is set */ 
		pmccadapriv->role = MCC_ROLE_STA;
	else {
		DBG_871X(FUNC_ADPT_FMT"\n", FUNC_ADPT_ARG(padapter));
		DBG_871X("Unknow:P2P state:%d, mlme state:0x%2x, mlmext info state:0x%02x\n", 
			pwdinfo->role, pmlmepriv->fw_state, padapter->mlmeextpriv.mlmext_info.state);
		res =  _FAIL;
	}

	if (res == _SUCCESS)
		rtw_hal_config_mcc_role_setting(padapter);
...
/*  
 Return value: ture for driver under MCC, false of driver not under MCC
 */
u8 rtw_hal_set_mcc_setting(PADAPTER padapter, u8 bstop)
{
	struct dvobj_priv	*pdvobjpriv = adapter_to_dvobj(padapter);
	struct mcc_obj_priv *pmccobjpriv = &(pdvobjpriv->mcc_objpriv);
	struct macid_ctl_t *pmacid_ctl = dvobj_to_macidctl(pdvobjpriv);
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;
	struct mlme_priv	*pbuddymlmepriv = &padapter->pbuddy_adapter->mlmepriv;
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_priv	*pbuddymlmeext = &padapter->pbuddy_adapter->mlmeextpriv;
	struct mcc_adapater_priv *pmccadapriv = &padapter->mcc_adapterpriv;
	struct mcc_adapater_priv *pbuddymccadapriv = &padapter->pbuddy_adapter->mcc_adapterpriv;
	u8 status = _FAIL;
	u16 trycnt = 100;
	u8 channel = pmlmeext->cur_channel; 
	u8 channel_offset = pmlmeext->cur_ch_offset;
	u8 bwmode = pmlmeext->cur_bwmode;

	DBG_871X(FUNC_ADPT_FMT": channel:%d, channel_offset:%d, bwmode:%d\n", FUNC_ADPT_ARG(padapter), channel, channel_offset, bwmode);
#if 0
	DBG_871X("HZ: %d\n", HZ);
	rtw_write8(padapter, 0x85, 0x5);
	rtw_write8(padapter->pbuddy_adapter, 0x85, 0x5);
#endif


	pmccobjpriv->mcc_c2h_status = MCC_RPT_MAX;
	rtw_sctx_init(&pmccobjpriv->mcc_sctx, MCC_EXPIRE_TIME);
	DBG_871X("init mcc_c2h_status:%d\n", pmccobjpriv->mcc_c2h_status);
	
	if(_FALSE == bstop) {
		u8 cur_mcc_success_cnt = 0;

		/* 35 for at least one cliente connecting to softap, 70 for no client connecting to softap */
		pmccobjpriv->duration = MCC_DURATION;

		/*We only check whether driver needs MCC if driver wants to start MCC*/
		if (rtw_hal_isneed_mcc(padapter) == _FALSE) {
			//pmccobjpriv->do_mcc = _FALSE;
			rtw_hal_clear_mcc_status(padapter, MCC_STATUS_DOING_MCC);
			goto exit;
		} else {
			pmccadapriv->mcc_tp = pbuddymccadapriv->mcc_tp = 0;
			pmccadapriv->mcc_tx_bytes_from_kernel= pbuddymccadapriv->mcc_tx_bytes_from_kernel= 0;
			pmccadapriv->mcc_last_tx_bytes_from_kernel= pbuddymccadapriv->mcc_last_tx_bytes_from_kernel= 0;
			pmccadapriv->mcc_tx_bytes_to_port= pbuddymccadapriv->mcc_tx_bytes_to_port= 0;
		}

		if((!rtw_hal_decide_mcc_role(padapter)) || (!rtw_hal_decide_mcc_role(padapter->pbuddy_adapter))) {
			//pmccobjpriv->do_mcc = _FALSE;
			//pmccobjpriv->need_mcc = _FALSE;
			rtw_hal_clear_mcc_status(padapter, (MCC_STATUS_DOING_MCC | MCC_STATUS_NEED_MCC));
			goto exit;
		}
		
		rtw_hal_set_mcc_macid(padapter, bstop);
		rtw_hal_set_mcc_macid(padapter->pbuddy_adapter, bstop);
		if (padapter->iface_type == IFACE_PORT0)
			rtw_hal_set_hwreg(padapter, HW_VAR_START_MCC_MODE, NULL);
		else 
			rtw_hal_set_hwreg(padapter->pbuddy_adapter, HW_VAR_START_MCC_MODE, NULL);
		
		DBG_871X( "wait for MCC C2H status for starting MCC\n" );

		if (!rtw_sctx_wait(&pmccobjpriv->mcc_sctx, __FUNCTION__)) {
			DBG_871X_LEVEL(_drv_always_, FUNC_ADPT_FMT" _rtw_down_sema(&pdvobjpriv->mcc_sema) return _FAIL, break\n", FUNC_ADPT_ARG(padapter));
			status = _FAIL;
		} else
			status = _SUCCESS;
		
		if (_FAIL == status) {
			DBG_871X( "Start MCC fail @@~~\n" );
			//pmccobjpriv->do_mcc = _FALSE;
			//pmccobjpriv->need_mcc = _FALSE; 
			rtw_hal_clear_mcc_status(padapter, (MCC_STATUS_DOING_MCC | MCC_STATUS_NEED_MCC));
			rtw_hal_set_mcc_macid(padapter, _TRUE);
			rtw_hal_set_mcc_macid(padapter->pbuddy_adapter, _TRUE);
			goto exit;
			
		} else {
			DBG_871X( "Start MCC successful\n");
			//pmccobjpriv->do_mcc = _TRUE;
			rtw_hal_set_mcc_status(padapter, MCC_STATUS_DOING_MCC);
		}
		...
		/*
		  stop MCC:
		  1. disconnect from AP or softap stop
		  2. scan
		  3. AP WOW or Sta WOW
		  */

		/* restore MEDIA_STATUS_RPT for unused macid (GO/AP) */
		if (pmccadapriv->role == MCC_ROLE_AP || pmccadapriv->role == MCC_ROLE_GO)
			rtw_hal_set_mcc_macid(padapter, bstop);
		else if (pbuddymccadapriv->role == MCC_ROLE_AP || pbuddymccadapriv->role == MCC_ROLE_GO)
			rtw_hal_set_mcc_macid(padapter->pbuddy_adapter, bstop);
			
		/* it mean adapter disconnect or scan*/
		rtw_hal_set_hwreg(padapter, HW_VAR_STOP_MCC_MODE, NULL);
		DBG_871X("wait for MCC C2H status for stopping MCC\n");
		...
		if (_FAIL == status) {
			DBG_871X( "Stop MCC fail @@~~\n" );
			//pmccobjpriv->do_mcc = _TRUE;
			rtw_hal_set_mcc_status(padapter, MCC_STATUS_DOING_MCC);
		} else {
			DBG_871X( "Stop MCC successful\n" );
			rtw_hal_clear_mcc_status(padapter, MCC_STATUS_DOING_MCC);
			/* switch to buddy's channel setting if buddy is linked */
			set_channel_bwmode(padapter, pbuddymlmeext->cur_channel, 
					pbuddymlmeext->cur_ch_offset, pbuddymlmeext->cur_bwmode);
			pmccobjpriv->mcc_launch_time = 0;
			//pmccobjpriv->do_mcc = _FALSE ;
		}
	}
exit:
	return rtw_hal_check_mcc_status(padapter, MCC_STATUS_DOING_MCC);
}
static void rtw_hal_set_mcc_rsvdpage_cmd(_adapter *padapter)
{
	u8	u1H2CRsvdPageParm[H2C_MCC_LOCATION_LEN]={0};
	struct mcc_obj_priv *pmccobjpriv = &(adapter_to_dvobj(padapter)->mcc_objpriv);
	
	DBG_871X(FUNC_ADPT_FMT": NullData=%d, LocCTS=%d\n", 
		FUNC_ADPT_ARG(padapter), pmccobjpriv->mcc_locNulldata, pmccobjpriv->mcc_locCTS);
	
	u1H2CRsvdPageParm[0] = pmccobjpriv->mcc_locCTS;
	u1H2CRsvdPageParm[1] = pmccobjpriv->mcc_locNulldata;

	rtw_hal_fill_h2c_cmd(padapter, H2C_MCC_LOCATION, H2C_MCC_LOCATION_LEN, u1H2CRsvdPageParm);
}
...
static void rtw_hal_set_mcc_noa_cmd(PADAPTER padapter)
{
	...
	u8 swchannel_early_time = MCC_SWCH_FW_NOTIFY_TIME;
	u8 i = 0;
...
static void rtw_hal_update_mcc_h2c_cmd(_adapter *padapter, u8 mask, u8 *upate_param)
{
	u8 u1UpdateH2CMccInfo[H2C_MCC_UPDATE_INFO_LEN] ={0};
	struct mcc_update_info_param *pupdate_info = (struct mcc_update_info_param *)u1UpdateH2CMccInfo;

	DBG_871X(FUNC_ADPT_FMT": upate_param[0]=0x%2x, upate_param[1]=0x%2x\n", 
		FUNC_ADPT_ARG(padapter), upate_param[0], upate_param[1]);
	_rtw_memset(u1UpdateH2CMccInfo, 0, H2C_MCC_UPDATE_INFO_LEN);

	
	pupdate_info->order = padapter->mcc_adapterpriv.order;
	if (mask & BIT0) {
		pupdate_info->mask_duration = 1;
		pupdate_info->duration = upate_param[0];
	}
	if (mask & BIT1) {
		pupdate_info->mask_InCurCh = 1;
		pupdate_info->InCurCh = upate_param[1] & 0x01;
	}
	if (mask & BIT2) {
		pupdate_info->mask_distxnull = 1;
		pupdate_info->distxnull = upate_param[1] & 0x02;
	}
	if (mask & BIT3) {
		pupdate_info->mask_c2hrpt = 1;
		pupdate_info->distxnull = upate_param[1] & 0x0C;
	}

#ifdef CONFIG_MCC_MODE_DEBUG
		rtw_hal_dump_update_mcc_info(pupdate_info);
#endif /* CONFIG_MCC_MODE_DEBUG */
	rtw_hal_fill_h2c_cmd(padapter, H2C_MCC_UPDATE_PARAM, H2C_MCC_UPDATE_INFO_LEN, u1UpdateH2CMccInfo);	
}

static void rtw_hal_set_mcc_h2c_cmd(_adapter *padapter, u8 bstop)
{
	uint mac_id=0;
	u8 u1H2CMccInfo[H2C_MCC_INFO_LEN] ={0};
	u8 u1H2CMccMacIdBitmap[H2C_MCC_MACID_BITMAP_LEN] = {0};
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	struct macid_ctl_t *macid_ctl = dvobj_to_macidctl(dvobj);
	struct mcc_obj_priv *pmccobjpriv = &(dvobj->mcc_objpriv);
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info	*pmlmeinfo = &pmlmeext->mlmext_info;
	struct mcc_info_param *pmcc_info = (struct mcc_info_param *)u1H2CMccInfo;
	struct mcc_macid_bitmap *pmcc_macid_bmp = (struct mcc_macid_bitmap *)u1H2CMccMacIdBitmap;
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(padapter);

	DBG_871X(FUNC_ADPT_FMT": bstop=%d\n", FUNC_ADPT_ARG(padapter), bstop);

	pmcc_info->order = padapter->mcc_adapterpriv.order;
	if (_FALSE == bstop)
		pmcc_info->totalnum = GET_IFACE_NUMS(padapter);
	else 
		pmcc_info->totalnum = 0xf;
	
	pmcc_info->chidx = pmlmeext->cur_channel;
	/*
		BW Data2[1:0]: 0: 20MHz, 1: 40MHz, 2: 80MHz
   		BW40SC Data2[4:2]: 0: don't care, 1: lower, 2: upper, 3: duplicate
		BW80SC Data2[7:5]: 0: don't care, 1: lower, 2: upper, 3: duplicate
	*/
	pmcc_info->bw = pmlmeext->cur_bwmode;
	pmcc_info->bw40sc = pmlmeext->cur_ch_offset;

	if (CHANNEL_WIDTH_80 == pmcc_info->bw) {		
		u8 center_ch = rtw_get_center_ch(pmlmeext->cur_channel, pmlmeext->cur_bwmode, pmlmeext->cur_ch_offset);
		if (center_ch > pmlmeext->cur_channel)
			pmcc_info->bw80sc= HAL_PRIME_CHNL_OFFSET_LOWER;
		else if(center_ch < pmlmeext->cur_channel)
			pmcc_info->bw80sc = HAL_PRIME_CHNL_OFFSET_UPPER;
		else
			pmcc_info->bw80sc = HAL_PRIME_CHNL_OFFSET_DONT_CARE;
	}

	pmcc_info->duration = padapter->mcc_adapterpriv.mcc_duration;

	pmcc_info->role = padapter->mcc_adapterpriv.role;

	pmcc_info->InCurCh = 1;

	if (IS_HARDWARE_TYPE_8812(padapter))
		pmcc_info->rfetype = pHalData->RFEType; /* RFETYPE (only for 8812)*/
	else
		pmcc_info->rfetype = 0;

	/* STA/GC TX NULL data to inform AP/GC for ps mode */
	if (MCC_ROLE_GO == pmcc_info->role || MCC_ROLE_AP == pmcc_info->role) {		
		pmcc_info->distxnull = MCC_DISABLE_TX_NULL;
	} else 
		pmcc_info->distxnull = MCC_ENABLE_TX_NULL;

	pmcc_info->c2hrpt = MCC_C2H_REPORT_ALL_STATUS;
	pmcc_info->channelscan = MCC_CHIDX;

#ifdef CONFIG_MCC_MODE_DEBUG
		rtw_hal_dump_mcc_info(pmcc_info);
#endif /* CONFIG_MCC_MODE_DEBUG */
	rtw_hal_fill_h2c_cmd(padapter, H2C_MCC_INFO, H2C_MCC_INFO_LEN, u1H2CMccInfo);
}
 ...
 */
static void rtw_hal_set_fw_mcc_rsvd_page(_adapter *padapter)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(padapter);
	...
	if (padapter->iface_type == IFACE_PORT0) {
		...
		rtw_hal_set_hwreg(padapter->pbuddy_adapter, HW_VAR_H2C_FW_JOINBSSRPT, (u8 *)(&mstatus));
		if (check_buddy_fwstate(padapter, WIFI_AP_STATE) == _TRUE)
			issue_beacon(padapter->pbuddy_adapter, 0);
		else
			issue_beacon(padapter, 0);
...
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	struct mcc_adapater_priv *pmccadapriv = NULL;
	_adapter *iface = NULL;
...
/*	1. Download RSVD Pages including NULL datas & CTS for primary & secondary adapter individually.
 	2. Driver config H2C NOA parameter for GO/AP
 	3. Driver config H2C IQK parameter for primary & secondary adapter individually.
 	3. H2C cmd for indicating fw channel information for primary & secondary adapter respectively (MCC_Info h2c cmd)
 */
void	rtw_hal_start_mcc_mode(_adapter *padapter)
{

	DBG_871X("===============>" FUNC_ADPT_FMT" time: %d\n", 
		FUNC_ADPT_ARG(padapter), rtw_get_current_time());
#if 0
	DBG_871X("Port: %d\n", padapter->iface_type);
	DBG_871X("%s(time: %d) ============ >\n", __func__, rtw_get_current_time());
#endif

	if (!rtw_buddy_adapter_up(padapter)) {
		DBG_871X(FUNC_ADPT_FMT": buddy adapter is not up!!!", FUNC_ADPT_ARG(padapter));
		return;
	}
	...
	/* Step 3 */
	rtw_hal_mcc_IQK_offload(padapter);  /* GO/AP IQK value offload */
	rtw_hal_mcc_IQK_offload(padapter->pbuddy_adapter);  /* GC/STA IQK value offload */

	/* set mac id to fw */
	rtw_hal_set_mcc_macid_cmd(padapter);

	/* Step 3-1. */
	/* total=2, GO/AP is 1st order */
	rtw_hal_set_mcc_h2c_cmd(padapter, _FALSE); 

	/* Step 3-2. */
	/* total=2, buddy current network is 2nd order */
	rtw_hal_set_mcc_h2c_cmd(padapter->pbuddy_adapter, _FALSE); 
...
void	rtw_hal_stop_mcc_mode(_adapter* padapter)
{
	DBG_871X("%s(time: %d) ============ >\n", __func__, rtw_get_current_time());

	/* 
		when adapter disconnect, stop mcc mod
		set mcc info for buddy if buddy is under linked, total=0xf means stop mcc mode 
	*/
	rtw_hal_set_mcc_h2c_cmd(padapter->pbuddy_adapter, _TRUE); 
	
	DBG_871X("<============ %s(time: %d)\n", __func__, rtw_get_current_time());
}

void rtw_hal_mcc_status_check(_adapter* padapter)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	struct mcc_obj_priv *pmccobjpriv = &(dvobj->mcc_objpriv);
...
void rtw_hal_display_mcc_info(void *sel, struct dvobj_priv *dvobj)
{
	struct mcc_obj_priv *pmccobjpriv = &(dvobj->mcc_objpriv);
	struct mcc_adapater_priv *pmccadapriv = NULL;
...
#endif /*CONFIG_AP_WOWLAN*/
#ifdef CONFIG_MCC_MODE
		case HW_VAR_SET_MCC_RSVD_PAGE:
			rtw_hal_set_fw_mcc_rsvd_page(adapter);
			break;
		case HW_VAR_START_MCC_MODE:
			rtw_hal_start_mcc_mode(adapter);
			break;
		case HW_VAR_STOP_MCC_MODE:
			rtw_hal_stop_mcc_mode(adapter);		
			break;
#endif /* CONFIG_MCC_MODE */
		default:
			if (0)
	...
	struct file *fp)
{
	...
	if (rtw_check_invalid_mac_address(pEEPROM->mac_addr, _TRUE) == _TRUE) {
...
void dm_DynamicUsbTxAgg(_adapter *padapter, u8 from_timer)
{
	...
	if(IS_HARDWARE_TYPE_8821U(padapter) )//|| IS_HARDWARE_TYPE_8192EU(padapter))
	{
		...
		if((pHalData->UsbRxAggMode == USB_RX_AGG_USB) && (check_fwstate(pmlmepriv, _FW_LINKED)== _TRUE))
		{
			...
			if(pdvobjpriv->traffic_stat.cur_tx_tp > 70 && pdvobjpriv->traffic_stat.cur_rx_tp < 30)			
				rtw_write16(padapter, REG_RXDMA_AGG_PG_TH,0x2005);
			else if(pdvobjpriv->traffic_stat.cur_tx_tp > 2 && pdvobjpriv->traffic_stat.cur_rx_tp < 30)
			...
			if(!rtw_hal_check_mcc_status(padapter, MCC_STATUS_DOING_MCC)) {
				...
				if (pmlmeextpriv->cur_bwmode >= pbuddymlmeextpriv->cur_bwmode) {
					switch(pbuddymlmeextpriv->cur_bwmode) {
						case CHANNEL_WIDTH_80:
							if(pdvobjpriv->traffic_stat.cur_rx_tp > 50 && pdvobjpriv->traffic_stat.cur_tx_tp < 60)
								rtw_write16(padapter , REG_RXDMA_AGG_PG_TH , 0x2005);
							else
								rtw_write16(padapter , REG_RXDMA_AGG_PG_TH , 0x1005);//0x2005,0x1C06,0x1A06;
							break;
							
	                                        case CHANNEL_WIDTH_40:
							if(pdvobjpriv->traffic_stat.cur_rx_tp > 50 && pdvobjpriv->traffic_stat.cur_tx_tp < 45)
                                               	rtw_write16(padapter , REG_RXDMA_AGG_PG_TH , 0x2005);
							else
								rtw_write16(padapter , REG_RXDMA_AGG_PG_TH , 0x1C06);
                                            	break;
						case CHANNEL_WIDTH_20:
							
							if(pdvobjpriv->traffic_stat.cur_rx_tp > 50 && pdvobjpriv->traffic_stat.cur_tx_tp < 20)
                                               	rtw_write16(padapter , REG_RXDMA_AGG_PG_TH , 0x2005);
                                         	else
								rtw_write16(padapter , REG_RXDMA_AGG_PG_TH , 0x1006);
							break;
					...
					switch(pmlmeextpriv->cur_bwmode) {
                                		case CHANNEL_WIDTH_80:
							if(pdvobjpriv->traffic_stat.cur_rx_tp > 50 && pdvobjpriv->traffic_stat.cur_tx_tp < 60)
								rtw_write16(padapter , REG_RXDMA_AGG_PG_TH , 0x2005);
                                     		else
