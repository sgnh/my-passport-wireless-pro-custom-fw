...
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	u4Byte			value32 =0;
	u1Byte          cnt, IGI_Pause = 0x7f, IGI_Resume = 0x20, IGI = 0x3e;   //IGI = 0x50 for cal EDCCA lower bound
...
)
{
	...
	if (pDM_Odm->SupportICType & ODM_IC_11N_SERIES)
	{
		...
		ODM_SetBBReg(pDM_Odm, ODM_REG_DBG_RPT_11AC, bMaskDWord, 0x209);
		ODM_SetBBReg(pDM_Odm, rFPGA0_XA_LSSIReadBack, BIT1|BIT0, 0x3);
	}
	...
	if (pDM_Odm->SupportICType & ODM_RTL8814A)	/* 8814a no need to find pwdB lower bound, maybe */
	{
        ...
        while (pDM_Odm->Adaptivity_IGI_upper <= 0x26 && count < 3) {
