...
static int rtw_cfg80211_ap_set_encryption(struct net_device *dev, struct ieee_param *param, u32 param_len)
{
	...
	DBG_871X("%s, %d, param->u.crypt.alg = %s  \n", __func__, __LINE__, param->u.crypt.alg);
...
static int rtw_cfg80211_set_wpa_ie(_adapter *padapter, u8 *pie, size_t ielen)
{
	...
	{
		if(rtw_parse_wpa2_ie(pwpa2, wpa2_ielen+2, &group_cipher, &pairwise_cipher, NULL, &group_mgmt_cipher) == _SUCCESS)
		{
			padapter->securitypriv.dot11AuthAlgrthm= dot11AuthAlgrthm_8021X;
	...
	*cookie = (unsigned long) buf;
	...
	if (IEEE80211_STYPE_PROBE_RESP == frame_styp) {
		...
		if (MLME_IS_AP(padapter)) {
			wait_ack = 0;
			issue_probersp(padapter, GetAddr1Ptr(buf), _FALSE);
			goto exit;
		} else {
			wait_ack = 0;
			goto dump;
