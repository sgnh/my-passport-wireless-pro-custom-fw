...
_func_enter_;
	...
	if (pxmitbuf->triggered_by_pspoll == 1) {
		...
		struct sta_info *psta = pxmitbuf->psta;

		if (psta)
			ATOMIC_DEC(&psta->pspoll_op);
...
int usb_napi_recv(_adapter *padapter, int budget)
{
	...
	//indicate packets in the rx_napi_skb_queue if "work_done==budget" in previous recv_napi_poll()
	while ( (work_done < budget) &&
		...
		if ((padapter->bDriverStopped == _TRUE)||(padapter->bSurpriseRemoved== _TRUE)) {
			DBG_8192C("usb_napi_recv => bDriverStopped or bSurpriseRemoved \n");
			rtw_skb_free(pskb);
			goto exit;
			break;
		}
...
#ifdef CONFIG_USE_USB_BUFFER_ALLOC_RX
	//receive packets from recv_buf_pending_queue and insert data packets into rx_napi_skb_queue.
	while (NULL != (precvbuf = rtw_dequeue_recvbuf(&precvpriv->recv_buf_pending_queue)))
	{
		if ((padapter->bDriverStopped == _TRUE)||(padapter->bSurpriseRemoved== _TRUE))
		{
...
#else
	//receive packets from rx_skb_queue and insert data packets into rx_napi_skb_queue.
	while (NULL != (pskb = skb_dequeue(&precvpriv->rx_skb_queue))) {
		...
		recvbuf2recvframe(padapter, pskb);
		//DBG_8192C("usb_napi_recv: work_done=%d", work_done);
...
#ifdef CONFIG_USE_USB_BUFFER_ALLOC_RX
			!_rtw_queue_empty(&precvpriv->recv_buf_pending_queue))
#else
