...
static ssize_t proc_set_change_bss_chbw(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		if ((ch <= 14
			&& ((ch > 9 && offset == HAL_PRIME_CHNL_OFFSET_LOWER) || (ch < 5 && offset == HAL_PRIME_CHNL_OFFSET_UPPER)))
				||((offset == HAL_PRIME_CHNL_OFFSET_DONT_CARE) && (bw != CHANNEL_WIDTH_20) )
					|| ((bw == CHANNEL_WIDTH_20) && (offset != HAL_PRIME_CHNL_OFFSET_DONT_CARE))){
			DBG_871X("ch bw offset(%d,%d,%d) setting violate spec\n", ch, bw, offset);
			goto exit;
		}
		
		if (check_fwstate(mlme, WIFI_AP_STATE) && check_fwstate(mlme, WIFI_ASOC_STATE))
			start_bss_network_by_proc(adapter, ch, bw, offset);
	}
...
static int proc_get_bss_chbw(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
...
const struct rtw_proc_hdl adapter_proc_hdls [] = {
	...
	{"adapter_status", proc_get_dump_adapters_status, NULL},
...
//	{"rssi_disp",proc_get_rssi_disp, proc_set_rssi_disp},
	...
	{"chbw_info", proc_get_bss_chbw, proc_set_change_bss_chbw},
};
