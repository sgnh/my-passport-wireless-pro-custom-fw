...
#ifdef CONFIG_IEEE80211W
void _clnt_sa_query_timer_hdl (void *FunctionContext)
{
	_adapter *padapter = (_adapter *)FunctionContext;
	clnt_sa_query_timer_hdl(padapter);
}

void _ap_sa_query_timer_hdl (void *FunctionContext)
{
	struct sta_info *psta = (struct sta_info *)FunctionContext;
	ap_sa_query_timer_hdl(psta);
}

void init_dot11w_expire_timer(_adapter *padapter, struct sta_info *psta)
{
	_init_timer(&psta->dot11w_expire_timer, padapter->pnetdev, _ap_sa_query_timer_hdl, psta);
}
...
#ifdef CONFIG_IEEE80211W
	_init_timer(&pmlmeext->sa_query_timer, padapter->pnetdev, _clnt_sa_query_timer_hdl, padapter);
#endif //CONFIG_IEEE80211W
