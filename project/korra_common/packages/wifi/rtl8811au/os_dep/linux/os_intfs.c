...
static int rtw_net_set_mac_address(struct net_device *pnetdev, void *addr)
{
	...
	//Also, it can use "pnetdev->flags & IFF_UP" to check interface state
	if (padapter->netif_up == _FALSE)
	{
		if (rtw_check_invalid_mac_address(sa->sa_data, _FALSE) == _FALSE) 
		{
			DBG_871X(FUNC_ADPT_FMT": Set Mac Addr to "MAC_FMT" Successfully\n"
				,FUNC_ADPT_ARG(padapter), MAC_ARG(sa->sa_data));
			_rtw_memcpy(padapter->eeprompriv.mac_addr, sa->sa_data, ETH_ALEN); //set mac addr to adapter
...
#ifdef CONFIG_NAPI
	DBG_871X("Support NAPI\n");
#ifdef CONFIG_GRO
	DBG_871X("Support GRO\n");
#endif
#endif
...
int _netdev_if2_open(struct net_device *pnetdev)
{
		...
		primary_padapter->hw_init_completed == _TRUE)
	{
		...
		{
			DBG_871X("_netdev_if2_open: napi_enable\n");
...
netdev_if2_open_error:
	...
	padapter->bup = _FALSE;
       ...
       {
               DBG_871X("_netdev_if2_open: napi_disable\n");
...
#ifdef CONFIG_MCC_MODE
	padapter->mcc_adapterpriv.order = 1;
#endif /* CONFIG_MCC_MODE */
...
int _netdev_open(struct net_device *pnetdev)
{
	...
	if(padapter->bup == _FALSE)
	{
		...
		if(padapter->napi_state==NAPI_DISABLE)
		{
			DBG_871X("_netdev_open: napi_enable\n");
			napi_enable(&padapter->napi);
			padapter->napi_state = NAPI_ENABLE;
		}
#endif
...
netdev_open_error:
	...
	if(padapter->napi_state==NAPI_ENABLE)
	{
		DBG_871X("_netdev_open: napi_disable\n");
		napi_disable(&padapter->napi);
		padapter->napi_state = NAPI_DISABLE;
	}
#endif
...
#ifdef CONFIG_MCC_MODE
	/* 0. Disable MCC if under MCC mode*/
	if (_TRUE == padapter->registrypriv.en_mcc) {
		...
		if (rtw_hal_check_mcc_status(padapter, MCC_STATUS_NEED_MCC)) {
			if (rtw_hal_check_mcc_status(padapter, MCC_STATUS_DOING_MCC)) {
				DBG_871X(FUNC_ADPT_FMT": stop mcc mode under AP WOW!\n", FUNC_ADPT_ARG(padapter));

				/* return _FALSE for stopping sucessfully */
				if(_FALSE == rtw_hal_set_mcc_setting(padapter->pbuddy_adapter,  _TRUE)) {
					//pmccobjpriv->need_mcc = _FALSE;
					rtw_hal_clear_mcc_status(padapter, MCC_STATUS_NEED_MCC);
					DBG_871X(FUNC_ADPT_FMT": driver stop sucessfully when entering AP WOW(0x%02x)!\n", 
						FUNC_ADPT_ARG(padapter), pmccobjpriv->mcc_status);
				}
			} else {
				//pmccobjpriv->need_mcc = _FALSE;
				rtw_hal_clear_mcc_status(padapter, MCC_STATUS_NEED_MCC);
				DBG_871X(FUNC_ADPT_FMT": driver is not under MCC mode when entering AP WOW(0x%02x)!\n", 
					FUNC_ADPT_ARG(padapter), pmccobjpriv->mcc_status);
			}
		} else {
