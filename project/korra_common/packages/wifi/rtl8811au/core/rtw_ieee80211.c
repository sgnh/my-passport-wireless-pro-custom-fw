...
int rtw_parse_wpa2_ie(u8* rsn_ie, int rsn_ie_len, int *group_cipher, int *pairwise_cipher, int *is_8021x, int *group_mgmt_cipher)
{
	int i, ret=_SUCCESS;
	int left, count;
	u8 *pos, *BIP_pos, *last;
	...
	//the last byte of rsn ie
	last = rsn_ie+2+rsn_ie_len;
	
	//try to get BIP position if it is exist
	BIP_pos = pos;
	//4 bytes is Group Data Cipher Suite
	BIP_pos += 4;
	//DBG_871X("BIP_pos =%p, %p ##############\n", BIP_pos, last);
	if(BIP_pos < last) {
		//PSC_cnt is Pairwise Cipher Suite List
		PSC_cnt =  (u16 *) BIP_pos;
		*PSC_cnt = le16_to_cpu(*PSC_cnt);
		//2 bytes are Pairwise Cipher Suite Count
		BIP_pos += 2 + 4*(*PSC_cnt);
	}
	//DBG_871X("BIP_pos =%p, %p ##############\n", BIP_pos, last);
	if(BIP_pos < last) {
		//AKM_cnt is AKM Suite List
		AKM_cnt = (u16 *) BIP_pos;
		*AKM_cnt = le16_to_cpu(*AKM_cnt);
		//2 bytes are AKM Suite Count
		BIP_pos += 2 + 4*(*AKM_cnt);
	}
	//DBG_871X("BIP_pos =%p, %p ##############\n", BIP_pos, last);
	//2 bytes are RSN Capabilities
	if(BIP_pos < last) {
		//DBG_871X("RSN capability =%02x %02x ##############\n", *BIP_pos, *(BIP_pos+1));
		*group_mgmt_cipher = *BIP_pos & (BIT(6) | BIT(7));
	}
...
int rtw_get_cipher_info(struct wlan_network *pnetwork)
{
	u32 wpa_ielen;
	unsigned char *pbuf;
	int group_cipher = 0, pairwise_cipher = 0, is8021x = 0, group_mgmt_cipher = 0;
	int ret = _FAIL;
	...
	if(pbuf && (wpa_ielen>0)) {
		...
		if(pbuf && (wpa_ielen>0)) {
			...
			if (_SUCCESS == rtw_parse_wpa2_ie(pbuf, wpa_ielen+2, &group_cipher, &pairwise_cipher, &is8021x, &group_mgmt_cipher)) {
