...
#ifdef CONFIG_MCC_MODE
			/* To void not recieving station packet, so enlarge expire time */
			if(rtw_hal_check_mcc_status(padapter, MCC_STATUS_DOING_MCC))
				psta->expire_to = 30; /* 1min */
			else
...
if (chk_alive_num) {
	...
	struct mlme_ext_priv *pmlmeext = &padapter->mlmeextpriv;
	/* switch to correct channel of current network  before issue keep-alive frames(don't process channel setting under MCC mode) */
#ifdef CONFIG_MCC_MODE
	if (!rtw_hal_check_mcc_status(padapter, MCC_STATUS_DOING_MCC)) {
#endif /* CONFIG_MCC_MODE */
	...
	while(1)
	{
		...
		{		
			...
			if (delta_time < lifetime) {
				...
				if (pbuf == NULL) // HT CAP INFO IE don't exist, it is b/g mode bss.
				{
...
void start_bss_network_by_proc(_adapter *padapter, u8 ch, u8 bw, u8 offset)
{

	u8 *p;
	int ie_len;
	u8 cur_channel = ch, cur_bwmode = bw, cur_ch_offset = offset, operation_len = 0, center_freq = 0;
	struct registry_priv	 *pregpriv = &padapter->registrypriv;
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	struct security_priv* psecuritypriv=&(padapter->securitypriv);	
	WLAN_BSSID_EX *pnetwork = (WLAN_BSSID_EX *)&pmlmepriv->cur_network.network;
	...
	if((cur_channel <= 14 && pmlmeext->cur_channel >= 36) ||
		(cur_channel >= 36 && pmlmeext->cur_channel <= 14))
		change_band = _TRUE;
#ifdef CONFIG_80211AC_VHT
	if (pregpriv->vht_enable >= 1) {
		...
		switch (cur_bwmode) {
			...
			case CHANNEL_WIDTH_20:
				...
				/* modify vht op ie */
				if ( pvht_op_ie && vht_op_ielen) {
					SET_VHT_OPERATION_ELE_CHL_WIDTH(pvht_op_ie+2, 0); 
...
#endif
#ifdef CONFIG_80211N_HT
	if (pregpriv->ht_enable == 1) {
		u8 *pht_cap_ie = NULL, *pht_info_ie;
		int ht_cap_ielen = 0, ht_info_ielen = 0;

		pht_info_ie = rtw_get_ie((pnetwork->IEs + sizeof(NDIS_802_11_FIXED_IEs)), _HT_ADD_INFO_IE_, &ht_info_ielen, (pnetwork->IELength - sizeof(NDIS_802_11_FIXED_IEs)));
		pht_cap_ie = rtw_get_ie((pnetwork->IEs + sizeof(NDIS_802_11_FIXED_IEs)), _HT_CAPABILITY_IE_, &ht_cap_ielen, (pnetwork->IELength - sizeof(NDIS_802_11_FIXED_IEs)));

		if (pht_cap_ie && ht_cap_ielen)
			pht_cap = (struct rtw_ieee80211_ht_cap *)(pht_cap_ie + 2);

		if(pht_info_ie && pht_info_ie)
			pht_info = (struct HT_info_element *)(pht_info_ie + 2);

		if (pht_cap) {
			pht_cap ->cap_info &= ~IEEE80211_HT_CAP_SUP_WIDTH;
			pht_cap ->cap_info &= ~IEEE80211_HT_CAP_SGI_40;
		}

		if(pht_info) {
			pht_info->infos[0] &= ~HT_INFO_HT_PARAM_SECONDARY_CHNL_OFF_MASK;
			pht_info->infos[0] &= ~HT_INFO_HT_PARAM_REC_TRANS_CHNL_WIDTH;
		}
...
void start_bss_network(_adapter *padapter, u8 *pbuf)
{
	...
	if( p && ie_len)
	{
		if(GET_VHT_OPERATION_ELE_CHL_WIDTH(p+2) >= 1) {
			...
			else if (cur_bwmode == CHANNEL_WIDTH_20) {
				/* modify VHT OP IE */
				SET_VHT_OPERATION_ELE_CHL_WIDTH(p + 2, 0);
				/* reset to 0 for VHT20 */
				SET_VHT_OPERATION_ELE_CHL_CENTER_FREQ1(p + 2, 0);
				SET_VHT_OPERATION_ELE_CHL_CENTER_FREQ2(p + 2, 0);
	...
	if(_TRUE == pmlmeext->bstart_bss)
	{
	        if ((ATOMIC_READ(&pmlmepriv->olbc) == _TRUE)
				|| (ATOMIC_READ(&pmlmepriv->olbc_ht) == _TRUE)) //AP is not starting a 40 MHz BSS in presence of an 802.11g BSS.
		{
...
int rtw_check_beacon_data(_adapter *padapter, u8 *pbuf,  int len)
{
	...
	uint ie_len = 0;
	int group_cipher, pairwise_cipher, group_mgmt_cipher;	
	...
	if(p && ie_len>0)
	{
		if(rtw_parse_wpa2_ie(p, ie_len+2, &group_cipher, &pairwise_cipher, NULL, &group_mgmt_cipher) == _SUCCESS)
		{
			...
			psecuritypriv->wpa2_group_mgmt_cipher = group_mgmt_cipher;
#if 0
			switch(group_cipher)
			{
	...
	if(p && len>0)
	{
		...
		if ((pmlmepriv->sw_to_20mhz == 0) && (pmlmeext->cur_channel <= 14)) {
				...
				|| (pmlmepriv->ht_intolerant_ch_reported == _TRUE) || (ATOMIC_READ(&pmlmepriv->olbc) == _TRUE))
			{
				...
				// ?TODO : cur_bwmode/cur_ch_offset switchs to 20Mhz
			}
		}
		else {
...
void rtw_process_public_act_bsscoex(_adapter *padapter, u8 *pframe, uint frame_len)
{
	...
	if (frame_body_len > 8) // if EID_BSSIntolerantChlReport ie exists
	{
...
void concurrent_set_ap_chbw(_adapter *padapter, u8 channel, u8 channel_offset, u8 bwmode)
{
	...
	else if(check_buddy_fwstate(padapter, _FW_LINKED)==_TRUE)
	{
		...
		if (_TRUE == padapter->registrypriv.en_mcc) {
			...
			DBG_871X(FUNC_ADPT_FMT" start MCC mode under ap mode, CH=%d, offset=%d, BW=%d\n", 
					FUNC_ADPT_ARG(padapter), cur_channel, cur_ch_offset, cur_bwmode);
			DBG_871X(ADPT_FMT": CH=%d, offset=%d, BW=%d\n", ADPT_ARG(pbuddy_adapter), pbuddy_mlmeext->cur_channel, pbuddy_mlmeext->cur_ch_offset, pbuddy_mlmeext->cur_bwmode);
...
#endif /* CONFIG_MCC_MODE */
		{
			...
			if(cur_bwmode == CHANNEL_WIDTH_40)
			{
				...
				{
					cur_ch_offset = pbuddy_mlmeext->cur_ch_offset;
					if (cur_bwmode < pbuddy_mlmeext->cur_bwmode)
