...
static void _rtw_wait_scan_done(_adapter *adapter, u8 abort, u32 timeout_ms)
{
	u32 cnt=0;
	u32 start;
	struct mlme_priv	*pmlmepriv = &(adapter->mlmepriv);
...
{
	_rtw_wait_scan_done(adapter, _TRUE, 200);
}
...
_func_enter_;
	...
	if(psta)
	{
		...
		{			
			...
			updated = ap_free_sta(adapter, psta, _TRUE, WLAN_REASON_PREV_AUTH_NOT_VALID);
		}
...
_func_enter_;	
	...
	if(mac_id>=0){
		...
		#ifdef CONFIG_MCC_MODE 
			/* if report disconnect stauts to FW, we will not tx mgmt frame under data queue under MCC mode  */
			/* FW seting macid drop, so we connect again */
			if (adapter->registrypriv.en_mcc) {
				if (rtw_hal_check_mcc_status(adapter, MCC_STATUS_NEED_MCC)) {
					media_status = (mac_id<<8)|1; //  MACID|OPMODE:0 means connect
					rtw_hal_set_hwreg(adapter, HW_VAR_H2C_MEDIA_STATUS_RPT, (u8 *)&media_status);
				}
			}
