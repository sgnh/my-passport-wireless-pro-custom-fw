		...
		struct beacon_keys *recv_beacon)
{
	...
	if (elems.rsn_ie && elems.rsn_ie_len) {
		int group_mgmt_cipher = 0;
		recv_beacon->encryp_protocol = ENCRYP_PROTOCOL_WPA2;
		rtw_parse_wpa2_ie(elems.rsn_ie - 2, elems.rsn_ie_len + 2,
			&recv_beacon->group_cipher, &recv_beacon->pairwise_cipher,
			&recv_beacon->is_8021x, &group_mgmt_cipher);
	}
	/* checking WPA secon */
...
int rtw_check_bcn_info(ADAPTER *Adapter, u8 *pframe, u32 packet_len)
{
	...
	WLAN_BSSID_EX *bssid;
	int group_cipher = 0, pairwise_cipher = 0, is_8021x = 0, group_mgmt_cipher = 0;
	unsigned char *pbuf;
	u32 wpa_ielen = 0;
	...
	if (encryp_protocol == ENCRYP_PROTOCOL_WPA || encryp_protocol == ENCRYP_PROTOCOL_WPA2) {
		...
		if(pbuf && (wpa_ielen>0)) {
			...
			if(pbuf && (wpa_ielen>0)) {
				if (_SUCCESS == rtw_parse_wpa2_ie(pbuf, wpa_ielen+2, &group_cipher, &pairwise_cipher, &is_8021x, &group_mgmt_cipher)) {
					RT_TRACE(_module_rtl871x_mlme_c_,_drv_info_,
 ...
 * locally_bit_enable: check locally bit or not.
 *
 * Output:
 ...
 * Auther: Isaac.Li
 */
u8 rtw_check_invalid_mac_address(u8 *mac_addr, u8 locally_bit_enable) {
