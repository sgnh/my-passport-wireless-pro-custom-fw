	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (iface) {
			DBG_871X_SEL_NL(sel, "%2d %-8s "MAC_FMT" %4hhu %3u,%u,%u "MLME_STATE_FMT" %s%s\n"
				...
				, (iface->bDriverStopped == _TRUE)?" SR":""
				, (iface->bSurpriseRemoved == _TRUE)?" DS":""
			);
		}
	}
	...
	for(i=0; i< NUM_STA; i++)
	{
		...
		while ((rtw_end_of_queue_search(phead, plist)) == _FALSE)
		{
			...
			//if(extra_arg == psta->aid)
			{
				...
				DBG_871X_SEL_NL(m, "agg_enable_bitmap=%x, candidate_tid_bitmap=%x\n", psta->htpriv.agg_enable_bitmap, psta->htpriv.candidate_tid_bitmap);
#endif //CONFIG_80211N_HT
				DBG_871X_SEL_NL(m, "sleepq_len=%d, pspoll_op=%d\n", psta->sleepq_len, ATOMIC_READ(&psta->pspoll_op));
...
int proc_get_best_channel(struct seq_file *m, void *v)
{
	...
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct mlme_ext_priv *pmlmeext = &padapter->mlmeextpriv;
	u32 i, best_channel_24G = 1, best2nd_channel_24G = 1, best_channel_5G = 149, best2nd_channel_5G = 149, index_24G = 0, index_5G = 0;
	//extern int rtw_channel_plan;
	int korra_channel_plan, ingore_channel=0 ;
	
	//DBG_871X("!! rtw_channel_plan = %d\n", rtw_channel_plan);
	//DBG_871X("!! ChannelPlan = %d\n", padapter->mlmepriv.ChannelPlan);
	//rtw_channel_plan = padapter->mlmepriv.ChannelPlan;
	korra_channel_plan = padapter->mlmepriv.ChannelPlan;
	//DBG_871X("!! proc_get_best_channel ChannelPlan = %d\n", padapter->mlmepriv.ChannelPlan);
	DBG_871X("!! proc_get_best_channel korra_channel_plan = %d\n", korra_channel_plan);
	
	if(korra_channel_plan == 0x25 || korra_channel_plan == 0x29 || korra_channel_plan == 0x34 ||
		korra_channel_plan == 0x43 || korra_channel_plan == 0x7F || korra_channel_plan == 0x59 || korra_channel_plan == 0x54){
			ingore_channel = 1;
			best_channel_5G = 149;
			//DBG_871X("!! proc_get_best_channel ingore_channel = %d\n", ingore_channel);
			//DBG_871X("!! proc_get_best_channel best_channel_5G = %d\n", best_channel_5G);
	...
	//DBG_871X("!! proc_get_best_channel index_5G = %d\n", index_5G);
	for (i=0; (i < MAX_CHANNEL_NUM) && (pmlmeext->channel_set[i].ChannelNum !=0) ; i++) {
		// 2.4G
		if (pmlmeext->channel_set[i].ChannelNum >= 1 && pmlmeext->channel_set[i].ChannelNum <= 11) {
		...
		// 5G
		//DBG_871X("!! index_5G = %d\n", index_5G);
		//DBG_871X("!! ChannelNum = %d\n", pmlmeext->channel_set[i].ChannelNum);
		//DBG_871X("!! rx_count = %d\n", pmlmeext->channel_set[i].rx_count);
		//DBG_871X("!! channel_set rx_count = %d\n", pmlmeext->channel_set[index_5G].rx_count);
		if ( pmlmeext->channel_set[i].ChannelNum >= 36
			&& pmlmeext->channel_set[i].ChannelNum < 140 && ingore_channel != 1) {
			 // Find primary channel
			if ( (( pmlmeext->channel_set[i].ChannelNum - 36) % 4 == 0)
				&& (pmlmeext->channel_set[i].rx_count < pmlmeext->channel_set[index_5G].rx_count) ) {
				index_5G = i;
				best2nd_channel_5G = best_channel_5G;
		...
		if ( pmlmeext->channel_set[i].ChannelNum >= 149
			&& pmlmeext->channel_set[i].ChannelNum < 165) {
			 // find primary channel
			if ( (( pmlmeext->channel_set[i].ChannelNum - 149) % 4 == 0)
				&& (pmlmeext->channel_set[i].rx_count < pmlmeext->channel_set[index_5G].rx_count) ) {
				index_5G = i;
				best2nd_channel_5G = best_channel_5G;
				best_channel_5G = pmlmeext->channel_set[i].ChannelNum;
				//DBG_871X("!!!new best channel = %d\n", pmlmeext->channel_set[i].ChannelNum);
			}
		}
#if 1 // debug
	...
	DBG_871X_SEL_NL(m, "best2nd_channel_5G = %d\n", best2nd_channel_5G);
	DBG_871X_SEL_NL(m, "best_channel_24G = %d\n", best_channel_24G);
	DBG_871X_SEL_NL(m, "best2nd_channel_24G = %d\n", best2nd_channel_24G);
				...
				if(!_rtw_memcmp(get_my_bssid(&(pmlmeinfo->network)), &mac_addr[index][0], ETH_ALEN) 
					...
					if(psta)
					{
						...
						{			
							...
							updated = ap_free_sta(padapter, psta, _FALSE, 0);
...
int proc_get_mcc_info(struct seq_file *m, void *v)
{
	...
	rtw_hal_display_mcc_info(m, adapter_to_dvobj(adapter));
	return 0;
}
