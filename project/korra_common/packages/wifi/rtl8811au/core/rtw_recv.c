...
sint validate_recv_ctrl_frame(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if(GetFrameSubType(pframe) == WIFI_PSPOLL)
	{
		...
		if ((pspoll_op = ATOMIC_READ(&psta->pspoll_op)) >= 1) {
			
			DBG_871X("%s, pspoll_op=%d\n", __func__, pspoll_op);
		...
		if((psta->state&WIFI_SLEEP_STATE) && (pstapriv->sta_dz_bitmap&BIT(psta->aid)))
		{
			...
			{			
				...
				pxmitframe->attrib.triggered_by_pspoll = 1;

				ATOMIC_INC(&psta->pspoll_op);				
	...
	u8 *pphy_status)
{
	...
	pkt_info.bToSelf = (!pattrib->icv_err) && (!pattrib->crc_err)
		&& _rtw_memcmp(get_ra(wlanhdr), myid(&padapter->eeprompriv), ETH_ALEN);
	...
	if (_rtw_memcmp(myid(&padapter->eeprompriv), sa, ETH_ALEN) == _TRUE) {
		DBG_871X_LEVEL(_drv_always_, "Warning!!! sa equal my mac!!\n");
		rtw_warn_on(1);
	} else{
		pstapriv = &padapter->stapriv;
