...
void set_emmc_freq()
{
        REG32(SYS_PLL_EMMC2) = 0x078D7893;
        REG32(SYS_PLL_EMMC4) = 0x7; 
...
int dvrmain	( int argc, char * const argv[] )
{
	...
	if( flash_magicno == FLASH_MAGICNO_NAND )
	{
		...
		if ((*do_identify)(&device) < 0)
		{
			...
			NF_dump_flash(device, 0x03000000, 0, 0x2);
...
program_backup_copy_of_hwsetting:
	...
	#else
		#ifdef FOR_ICE_LOAD
        	prints("spi : erase 896KB from 0x");
        	print_hex(SPI_CODE_PART1);
        	prints("\n");
        #endif
		rtprintf("spi : erase 896KB from 0x%08x\n", SPI_CODE_PART1);
	    if ((*do_erase)(device, (unsigned int *)SPI_CODE_PART1, 0x000E0000) !=0 ) {
			return -3;
		}	
	#endif
#else
	...
	else if ( flash_magicno == FLASH_MAGICNO_EMMC)
	{
		...
		if(( programmed_fsbl_os_size != 0 )&&( programmed_fsbl_os_sig_size != 0 )) {
            ...
            #endif
			rtprintf("*******************************************\n");
            //get fsbl_os write block no. behind rescue kernel
