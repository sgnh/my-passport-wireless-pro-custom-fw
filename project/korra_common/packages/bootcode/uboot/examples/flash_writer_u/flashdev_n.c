...
#include "../../../arch/arm/include/asm/arch/io.h"
#include "cache.h"
...
static UINT8 nf_get_spare(INT8 mem_region, UINT32 *spare, UINT32 offset)
{
	...
	if (nf_spare_ddr_enabled()) {
		REG32(REG_SPR_DDR_CTL) = 0x30000000 | (PHYS(SPARE_DMA_ADDR+offset) >> 3);
		...
		else
		{
			if (verify_after_write) {
				...
				if ((temp != spare) || compare_memory(data_ptr, (UINT8 *)PAGE_TMP_ADDR, tmp_page_size)) {
					...
					//hexdump("[read]", PAGE_TMP_ADDR, device->PageSize);
					//prints("\n");
					
					//print_hex(spare);
					//prints("\n");
					//hexdump("[data]", data_ptr, device->PageSize);
...
static INT8 nf_find_blk(n_device_type *device, UINT32 start_block, UINT32 search_depth, UINT32 offset)
{
	...
	if (nf_spare_ddr_enabled()) {
		...
		rtd_outl(REG_SPR_DDR_CTL, rtd_inl(REG_SPR_DDR_CTL) & (~0x20000000));	// turn off spare to DDR
	}
	else
		spr_ddr_ctrl = 0;
...
static UINT8 nf_spare_ddr_enabled(void)
{
        return ((rtd_inl(REG_SPR_DDR_CTL) >> 29) & 0x1);
}
	...
	if ((signature) && (*start_blk < blk_state_len))
	{
		for (i = *start_blk; i < blk_state_len; i++)
		{
			...
			if (state == BLOCK_BAD) {
				...
				if (nf_spare_ddr_enabled()) {
					spr_ddr_ctrl = rtd_inl(REG_SPR_DDR_CTL);	// save original setting
					rtd_outl(REG_SPR_DDR_CTL, ~0x20000000);	// turn off spare to DDR
				}
				else
					spr_ddr_ctrl = 0;
