...
static int factory_check_sanity_from_SPI(uchar *buffer, int length)
{
	...
	for (i = 0;i < 2;i++)
	{
		...
		rtkspi_read32( (unsigned int)p_header1, factory_data_address, factory_header_bytes);
		   ...
		   ) 
		{
			...
			rtkspi_read32( (unsigned int)p_header2, factory_data_address, factory_header_bytes);
	...
	rtkspi_read32( (unsigned int)buffer, factory_data_address, pp_tarsize[factory_current_pp]);
...
static int factory_save_to_SPI(uchar* buffer, int len)
{
	...
	// write
	rtkspi_write8( factory_data_address, (unsigned int)buffer, total_len);

	{
		/* Write success */
		FAC_PRINTF("[FAC] Save to SPI (spi addr#:0x%x, buf:0x%08x, total length:0x%x)\n",			factory_data_address, (unsigned int)buffer, total_len);
