...
#else
	init_omap_revision();	// we no need
#ifdef CONFIG_SPL_BUILD
	if (warm_reset() && (omap_revision() <= OMAP5430_ES1_0))
		force_emif_self_refresh();
#endif
	watchdog_init();	//we no need
	set_mux_conf_regs(); // we no need
#ifdef CONFIG_SPL_BUILD
...
int print_cpuinfo(void)
{
	...
	puts("CPU  : Cortex-A9 dual core\n");
#elif defined(CONFIG_RTD1195)
	puts("CPU  : Cortex-A7 dual core\n");
#else
	puts("CPU  : ");
	omap_rev_string();
#endif
