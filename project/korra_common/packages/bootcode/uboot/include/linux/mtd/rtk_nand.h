...
#else
#define RTK_FLUSH_CACHE(addr, len) flush_cache(addr, len)
#endif
...
struct nand_chip {
	...
	int (*read_ecc_page) (struct mtd_info *mtd, u16 chipnr, unsigned int page, u_char *data, 
									u_char *oob_buf, u16 cp_mode, u16 cp_first, size_t cp_len);
...
extern int nand_read_ecc_on_the_fly (struct mtd_info *mtd, loff_t from, size_t len,size_t *retlen, u_char *buf, u_char *oob_buf, struct nand_oobinfo *oobsel,u16 cp_mode);
