...
#if defined(CONFIG_CMD_MM) || defined(CONFIG_CMD_NM)
...
# endif
	{
		...
		/* Patch for rbus or spi address read */
		if (addr < (RBUS_BASE_VIRT_OLD + RBUS_BASE_SIZE) && addr >= RBUS_BASE_VIRT_OLD)
		{
			addr = addr - RBUS_BASE_VIRT_OLD + RBUS_BASE_VIRT;
		}
		else if (addr < (SPI_FLASH_BASE_VIRT_OLD + SPI_FLASH_BASE_SIZE) && addr >= SPI_FLASH_BASE_VIRT_OLD)
		{
			addr = addr - SPI_FLASH_BASE_VIRT_OLD + SPI_FLASH_BASE_VIRT;
		}
#endif

                e_buffer(addr, (void*)addr, DISP_LINE_LEN, size, length, flag, DISP_LINE_LEN/size);
		addr += size*length;
	}
#endif
...
#if defined(CONFIG_RTD299X) || defined(CONFIG_RTD1195)
	/* Patch for rbus or spi address write */
	if (addr < (RBUS_BASE_VIRT_OLD + RBUS_BASE_SIZE) && addr >= RBUS_BASE_VIRT_OLD)
	{
		addr = addr - RBUS_BASE_VIRT_OLD + RBUS_BASE_VIRT;
	}
