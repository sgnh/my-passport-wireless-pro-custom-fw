...
start = get_timer(0);
	...
	while (get_timer(start) < delay && (!abort)){
        ...
        //regValue = regValue & 0xffff5ffc;
        //regValue = regValue | 0x5000;
        //rtd_outl(regAddr,regValue);
        //printf("MUXPAD0_reg addr %08x value %08x\n", regAddr, rtd_inl(regAddr));
        setISOGPIO(6,1);//SoC monitor
        debug("config %08x dir %08x dato %08x dati %08x\n", rtd_inl(regAddr), rtd_inl(dir), rtd_inl(dato), rtd_inl(dati));

#if defined(CONFIG_SYS_IR_SUPPORT)	
	
		if (!abort) {
                	printf("push KEY_LEFT : enter console mode\n");
			printf("push KEY_RIGHT: enter rescue linux\n");
			printf("push KEY_UP   : enter android recovery\n");
                	if (ir_flag == 1) {
				count = get_timer(0);  
	...
	if (!abort) {
                printf("======== Checking into USB update ====\n");	
		if (boot_ac_on == 1)
		{
...
# endif	/* CONFIG_BOOT_RETRY_TIME */
	...
	if (bootdelay >= 0 && s && !abortboot (bootdelay)) {
		...
		printf("[PMX] : GPIO check...\n");
		int regAddr = 0x18007310;
		int regValue = rtd_inl(regAddr);
		debug("[PMX] : Address %08x = %08x\n", regAddr, regValue);
		regValue = regValue & 0x3fff5fff;
		rtd_outl(regAddr, regValue);
		regValue = rtd_inl(regAddr);
		debug("[PMX] : Address %08x = %08x\n", regAddr, regValue);
		printf("[PMX] : Get GPIO 18 = %d\n", getISOGPIO(18));
		if(getISOGPIO(CONFIG_PMX_WD_BTM_GPIO_NUM) == 0){
			printf("[PMX] : Press battery status buttom, go into recovery mode...\n");
			if( run_command_list("bootp s", -1, 0) != 0){
				printf("[PMX] : Go into recovery mode fail...continue booting\n");
			}
			if( run_command_list("bootp r", -1, 0) != 0){
				printf("[PMX] : Go into recovery mode fail...continue booting\n");
			}
		}
#endif 
	...
	if (boot_mode != BOOT_RESCUE_MODE){
		...
		spiboot_failed = 1;
		/* enter boot from HDD mode because of booting failed or other reasons */	
		boot_mode = BOOT_NORMAL_MODE;
		run_command_list(s, -1, 0);
		/* enter rescue mode from SPI because of booting failed or other reasons */
		boot_mode = BOOT_RESCUE_MODE;
		setenv("rescue_cmd", "bootp s");
		s = getenv ("rescue_cmd");
		if (s)
			run_command_list(s, -1, 0);
		/* enter rescue mode from SPI because of booting failed or other reasons */
		boot_mode = BOOT_RESCUE_MODE;
		setenv("rescue_cmd", "bootp r");
		s = getenv ("rescue_cmd");
		if (s)
			run_command_list(s, -1, 0);
#endif
 ...
 ***********************************************************/
#ifdef CONFIG_BSP_REALTEK
	printf("[PMX] : Enter RTK recovery proceduree...\n");
