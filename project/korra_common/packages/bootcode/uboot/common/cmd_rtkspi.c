...
unsigned int    spiflash_size;

static void spi_switch_dual_read_mode(void)
{
    //printf("*** %s %d\n", __FUNCTION__, __LINE__);

    rtd_outl(SB2_SFC_OPCODE, 0x000004bb); //switch flash to read mode
    rtd_outl(SB2_SFC_CTL, 0x00000019); //command cycle
}
...
int rtkspi_identify( void )
{
    ...
    for (idx = 0; idx < DEV_SIZE_S; idx++)
    {
        if ( s_device[idx].id == id )
        {
            ...
            spiflash_size = pspi_flash_type->size;
            return idx;
        }
    }
...
void rtkspi_dual_read32( unsigned int target_address, unsigned int source_address, unsigned int byte_length )
{
...
int do_rtkspi(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    if( argc >= 2 ) {
        ...
        if( strncmp( argv[1], "dual_read", 9 ) == 0 && argc == 5 ) {
            cmd = 6;
        }
    }
    ...
    do {
        ...
        if( cmd == 1 || cmd == 2 || cmd == 6) { // read/write
    ...
    do {
        ...
        *((volatile unsigned int *)0x000000E0) = 0;
        ...
        if( cmd == 1 ) {       
            ...
            //printf("read - start:%lu ms, end:%lu ms, elapsed:%lu ms\n", start, end, elapsed); 
        }
        if( cmd == 2 ) {
        ...
        if( cmd == 6 ) {
            ...
            rtkspi_dual_read32( _iDDR_base_addr, _iSPI_base_addr, _iDataByteSize );

            //end = get_timer(0);
            //elapsed = end - start;

            //printf("dual_read - start:%lu ms, end:%lu ms, elapsed:%lu ms\n", start, end, elapsed);      
        }
#ifdef CONFIG_DTB_IN_SPI_NOR
        if( cmd == 4 ) {
...
U_BOOT_CMD(
    ...
    "rtkspi load dts kernel [DDR_addr]\n"
    "rtkspi load dts rescure [DDR_addr]\n"
    "      - SPI start address\n"
    "      - DDR start address ( suggest large than 0x01500000 )\n"
    "      - data bytes ( should 4-byte alignment )\n"
    "      - [DDDR_addr]optional address\n"
    "rtkspi dual_read 0x00000000 0x01500000 0x00100000\n"
);
