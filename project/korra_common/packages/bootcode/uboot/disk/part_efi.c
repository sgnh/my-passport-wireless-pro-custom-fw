				...
				disk_partition_t * info)
{
	...
	unsigned char *gpt_guid = gpt_pte[part - 1].unique_partition_guid.b;
	/* Translate 16byte GPT GUID into 36 chars */
	int i,x=0;
	/* First 3 parts are in little endian order */
	for(i=0; i<4; i++)
		snprintf(info->uuid+i*2, 3, "%02x", gpt_guid[3-i]);
        gpt_guid += 4;
        x++;
	for(i=4; i<6; i++)
		snprintf(info->uuid+i*2+x, 3, "%02x", gpt_guid[5-i]);
        gpt_guid += 2;
        x++;
	for(i=6; i<8; i++)
		snprintf(info->uuid+i*2+x, 3, "%02x", gpt_guid[7-i]);
        gpt_guid += 2;
        x++;
	/* Last 2 parts are in big endian order */
	for(i=8; i<10; i++)
		snprintf(info->uuid+i*2+x, 3, "%02x", gpt_guid[i-8]);
        gpt_guid += 2;
        x++;
	for(i=10; i<16; i++)
		snprintf(info->uuid+i*2+x, 3, "%02x", gpt_guid[i-10]);
	/* Insert '-' at 8, 12 , 16, 20 char */
	/* Set '-' as 8, 13 , 18, 23 char */
	info->uuid[8] = info->uuid[13] = info->uuid[18] = info->uuid[23] = '-';
