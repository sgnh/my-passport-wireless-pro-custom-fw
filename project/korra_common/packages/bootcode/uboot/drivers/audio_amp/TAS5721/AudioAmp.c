...
static unsigned long long m_PIN_951_RST=0, m_PIN_AMP_RESET=0;
#if 0
#if defined(BOARD_ID_RTD2974_HISENSE_V1)|| defined(BOARD_ID_RTD2974_HISENSE_V2) || defined(BOARD_ID_RTD2974_DEMO_V2_TPV)
static unsigned long long m_PIN_HEADPHONE_PLUG_IN=0;
static unsigned long long m_PIN_AMP_RESET=0;
#endif
#endif
...
void AudioAmp_InitAudioOut2I2S_1st(void)
{
	...
	if (pcb_mgr_get_enum_info_byname("PIN_AMP_RESET", &value) < 0) {
		printf("no PIN_AMP_RESET\n");
		m_PIN_AMP_RESET = 0;
	} else {
		m_PIN_AMP_RESET = value;
		printf("get m_PIN_AMP_RESET = %08x%08x\n", HI32(value), LO32(value));
	}
	//if (m_PIN_951_RST) {
		//reset Yamaha YYS951
		//IO_Set(m_PIN_951_RST, 1);
...
#if defined(BOARD_ID_RTD2974_DEMO_V2_TPV)
	if (m_PIN_AMP_RESET) {
		//reset AMP
		IO_Set(m_PIN_AMP_RESET, 0);
		udelay(300000);
		IO_Set(m_PIN_AMP_RESET, 1);
		udelay(300000);
	}
#endif
	//if (m_PIN_951_RST) {
...
//		udelay(100000);
	...
	////Brian-20130305-Fix I2C1 random write fail when boot up
	// DTV_RF_AGC, DTV_IF_AGC
	rtd_outl(0xb8000800, 0x3030f868);
	// TP2 Pin_Mux_Ctrl4
	rtd_outl(0xb80008D0, 0x00000001);
