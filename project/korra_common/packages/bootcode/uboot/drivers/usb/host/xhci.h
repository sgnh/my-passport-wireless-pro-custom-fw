...
#else
	wmb(); // add by cfyeh
	return rtd_inl((unsigned int)regs);
#endif
}
...
#else
	wmb(); // add by cfyeh
	rtd_outl((unsigned int)regs, val);
	wmb(); // add by cfyeh
#endif
}
...
#else
	rmb();
	u64 val_lo = rtd_inl((unsigned int)ptr);
	u64 val_hi = rtd_inl((unsigned int)(ptr + 1));
#endif
...
static inline void xhci_writeq(__le64 volatile *regs, const u64 val)
{
	...
	writel(val_hi, ptr + 1);
#else
	rtd_outl((unsigned int)ptr, val_lo);
	rtd_outl((unsigned int)(ptr + 1), val_hi);
	wmb();
#endif
}
